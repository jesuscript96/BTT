...
# Gap 30-50%, EOD Volume > 150M
where_m_base = "gap_pct >= 30.0 AND gap_pct <= 50.0 AND volume >= 150000000.0"

def run_query(m_filter):
    query = f"""
        WITH daily_base AS (
        ...
        SELECT count(*) FROM calculated WHERE {m_filter}
    """
    return con.execute(query).fetchall()[0][0]

count_no_pm = run_query(where_m_base)
print(f"Results without PM filter: {count_no_pm}")

count_with_pm = run_query(where_m_base + " AND pm_v >= 5000000")
print(f"Results with PM filter (5M): {count_with_pm}")
