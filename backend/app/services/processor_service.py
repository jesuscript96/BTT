...
def process_daily_metrics(df, con=None):
    ...
    for date, group in df.groupby("date"):
        ...
        pm_session = clean_group[
            (c_times >= pd.Timestamp("04:00").time()) & (c_times < pd.Timestamp("09:30").time())
        ]
        rth_session = clean_group[
            (c_times >= pd.Timestamp("09:30").time()) & (c_times < pd.Timestamp("16:00").time())
        ]
        ...
        # PM High Time
        pm_high_time = (
            pm_session.loc[pm_session["high"].idxmax()]["timestamp"].strftime("%H:%M")
            if not pm_session.empty and len(pm_session) > 0
            else None
        )
        ...
        pm_low_time = (
            pm_session.loc[pm_session["low"].idxmin()]["timestamp"].strftime("%H:%M")
            if not pm_session.empty and len(pm_session) > 0
            else None
        )
...
def get_aggregate_time_series(ticker_date_pairs):
    ...
    for item in ticker_date_pairs[:10]:
        ...
        df = con.execute(
            "SELECT timestamp, open, close FROM intraday_1m WHERE ticker = ? AND CAST(timestamp AS DATE) = CAST(? AS DATE)",
            [ticker, date],
        ).fetch_df()
