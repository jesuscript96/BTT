...
F2: the hot per-bar loop is JIT-compiled (Numba) in portfolio_sim_jit.py. This
module is now a THIN WRAPPER: it maps strings/None/partial-TP-lists into the
numeric arrays + int enums the JIT core needs, calls the core, then rebuilds the
exact same trade dicts and applies the daily locates fee. The public signature,
the returned dict, and every number are unchanged (Golden B tolerance 0).
"""
...
def simulate(
    ...
    # (backtest_service already passes float64/int64/bool; this is defensive and
    #  keeps a single compiled Numba signature. Values are unchanged.)
    ...
    # (replicates datetime.fromtimestamp(ts/1e9, utc).hour/.minute exactly)
    needs_hours = (tp_time_mode == 2) or bool(np.any(pt_type == _pjit.PT_HOUR))
    if needs_hours and has_timestamps:
        has_hours = True
        row_hours = np.empty(n, dtype=np.int64)
        row_minutes = np.empty(n, dtype=np.int64)
        for i in range(n):
            _dt = datetime.fromtimestamp(timestamps_a[i] / 1e9, tz=timezone.utc)
            row_hours[i] = _dt.hour
            row_minutes[i] = _dt.minute
    else:
        has_hours = False
    ...
    # --- rebuild the exact trade dicts (rounding done here, in Python, exactly
    #     as the original did — on np.float64 values) ---
