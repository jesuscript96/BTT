      ...
      {vista === "resumen" && (
        <>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "2px 16px", padding: "4px 0 8px" }}>
            ...
            {out.ticker_cap_report && <Stat label="Tope por acción" value={`${n(out.ticker_cap_report.skipped + out.ticker_cap_report.trimmed, 0)} trades`} sub={`fuera/recortados · ${n(out.ticker_cap_report.pct, 1)} % del día en ${out.ticker_cap_report.basis === "notional" ? "nocional" : "riesgo"}${out.ticker_cap_report.sin_stop > 0 ? ` · ${n(out.ticker_cap_report.sin_stop, 0)} sin stop` : ""}`} help="Trades que llegaron con su acción ya llena (otra estrategia dentro a la vez) y se quedaron fuera o recortados a lo que quedaba del tope. «Sin stop»: trades cuyo riesgo no se puede medir (la corrida no dimensionó por stop y no guardó uno): no consumen ni se topan." tone={(out.ticker_cap_report.skipped + out.ticker_cap_report.trimmed) > 0 ? "warning" : undefined} />}
