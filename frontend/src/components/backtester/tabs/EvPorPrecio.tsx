  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-3 py-2 flex items-center gap-2">
        <span className="text-[10px] font-semibold text-[var(--color-ec-text-primary)] uppercase tracking-[0.12em] ml-8 inline-flex items-center gap-1">
          ...
          <InfoTooltip
            position="left"
            width={380}
            text="Las tres medidas de la estrategia que se pueden enfrentar al «fade necesario» del locate (Costes opcionales → Puerta por EV/MFE/Fade). Todas en % del precio de ENTRADA (el fill, no el precio medio con las pirámides), brutas y sin que el capital las mueva; por trade, contando el trade entero (parciales incluidos). <b>EV</b>: media de lo que se movió el precio a favor desde la entrada hasta el precio medio de todas las salidas = WR × ganancia media − (1 − WR) × pérdida media, en unidades de fade. <b>MFE medio</b>: media de lo máximo que se movió a favor desde la entrada hasta la salida final. <b>Fade medio</b>: media de lo que se movió a favor desde la entrada hasta la salida FINAL (la última pierna). <b>Captura</b> = EV / MFE: qué parte del recorrido que hubo se llevan tus salidas. Si es baja, el problema son las salidas, no los locates; pero no se maximiza a ciegas (aguantar más sube la captura y puede hundir el win rate y el EV): lo que se maximiza es el EV, la captura dice dónde mirar. Va por comparación entre tramos y entre versiones de la estrategia; como orientación gruesa para un fade intradía con parciales por hora, entre el 20 y el 40 % es lo normal, por debajo del 15 % las salidas están lejos del movimiento, y por encima del 50 % es raro. <b>Tope según EV</b> (NO es lo que se pagó: la simulación cobra lo que le pongas en el rango) = EV (%) × precio de la acción, en $ POR PAQUETE de 100: el precio de paquete a partir del cual el locate se come el EV, porque el locate cuesta precio del paquete / 100 por acción y el EV da EV % × precio por acción. Con EV 4 %: 20 $ en una acción de 5 $, 4 $ en una de 1 $, 2 $ en una de 0,50 $. Por tramo va con el precio medio de entrada del tramo; el de arriba es «por cada 1 $ de precio». Es el equilibrio exacto, sin margen por comisiones ni por el redondeo a paquetes enteros (20 acciones pagan el paquete de 100): el margen que quieras dejarte lo pones tú. <b>Barras</b>: la medida elegida por tramos de 0,5 $ del precio de entrada; pasa el ratón para leer cada barra (las claras tienen menos de 20 trades: ruido). <b>Tabla</b>: las tres medidas en los seis tramos de la puerta «por rango» y del cuadro de mandos: son los números que se copian ahí."
          />
        </span>
      ...
      <div className="px-3 pb-1 -mt-1 flex items-center justify-end gap-1 text-[10px] font-mono text-[var(--color-ec-text-secondary)]">
        ...
        <InfoTooltip position="right" width={340} title="Tramo más flojo" text={tramoFlojo
              ...
              {datos.finas.map((b, k) => {
                ...
                return (
                  <div key={k} className="flex-1 min-w-0 relative" onMouseEnter={() => setHover(k)} onMouseLeave={() => setHover(null)}
                       title={`${etiquetaFina(b.lo)}${b.lo < TECHO ? ` – ${etiquetaFina(b.lo + PASO)}` : ""} $ · EV ${pct(b.ev)} · MFE ${pct(b.mfe)} · Fade ${pct(b.fade)} · tope según EV ${b.locMax == null ? "—" : f2(b.locMax) + " $/paq."} · n ${b.n}`}>
                    <div style={{
