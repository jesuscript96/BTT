...
export default function PerformanceTab({
  ...
  const gridData = useMemo(() => {
    ...
    // Base de cada periodo (PRD_02): equity al inicio del periodo
    // (init_cash + pnl de trades cerrados antes). El % del portfolio es
    // pnl/base. Componer los retornos por (ticker,día) como antes inflaba el
    // número con posiciones solapadas (cada total_return_pct de day_results se
    // calcula sobre la base COMPLETA del portfolio, no sobre una fracción).
  ...
  const renderCell = (cell: CellData) => {
    ...
    switch (metric) {
      case "PnL %":
        // PRD_02: % del periodo sobre la base de equity real (pnl/base), no la
        // composición de retornos por ticker-día (que disparaba el número).
  ...
  return (
    <div className="space-y-12" style={{ paddingTop: 24 }}>
      ...
      <div style={{ borderTop: '0.5px solid var(--color-ec-border)', paddingTop: 20, marginTop: 20 }}>
