...
export default function Home() {
  ...
  */
  ...
  const handleRunWithDraft = async (draft: Draft, paramsOverride?: any) => {
    ...
    // paramsOverride = parámetros que el panel pasó a handleRun al lanzar (T4).
    // Sin él (wizard/builder) se usa el último estado del panel, como antes.
    // Este camino llegaba al backend con defaults (10000/FIXED) ignorando lo
    // tecleado porque solo leía panelParamsRef.
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        // Restaurar la base de la corrida (T5): los refs no sobreviven a la
  ...
  useEffect(() => {
    ...
    const resultsState = {
      ...
      backtestParams: backtestParamsRef.current, // T5: risk_type/monthly_expenses/is_percent tras recargar
      activeStrategy,
      selectedDay,
      mode,
      builderDraft
  ...
  // ── Base $ de esta corrida (fuente de verdad, T6) ──
  ...
  const isFilteredResult = useMemo(() => {
    ...
    // "Days" en días de calendario únicos (T7): el backend reporta total_days
    // como pares (ticker, día). Con IS=100 el filtro de abajo no corre y el
    // valor crudo llegaba a la UI, así que la corrección se aplica también aquí.
  ...
  }, [result, currentIsPercent, runInitCash]);
