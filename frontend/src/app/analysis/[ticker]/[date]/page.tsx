...
export default function AnalysisPage() {
    ...
    return (
        <div className="min-h-screen bg-[var(--color-ec-bg-base)] flex flex-col font-sans">
            {/* Header */}
            <div className="p-6 bg-[var(--color-ec-bg-surface)] border-b border-[var(--color-ec-border)] flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl">
                <div className="flex items-center gap-6">
                    <Link href="/" className="p-2 hover:bg-[var(--color-ec-bg-surface)] rounded-full transition-colors text-[var(--color-ec-text-secondary)] hover:text-[var(--color-ec-text-high)]">
                        <ArrowLeft className="w-6 h-6" />
                    </Link>
                    <div className="flex items-center gap-4">
                        <div className="flex flex-col items-center justify-center p-2 bg-[var(--color-ec-bg-elevated)] rounded-lg w-16 h-16 border border-[var(--color-ec-border)]">
                            <span className="text-[10px] font-bold text-[var(--color-ec-text-muted)] leading-none">{month}</span>
                            <span className="text-2xl font-black text-[var(--color-ec-text-high)] leading-none my-0.5">{day}</span>
                            <span className="text-[10px] font-bold text-[var(--color-ec-text-muted)] leading-none">{year}</span>
                        </div>
                        <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                                <div className="w-2.5 h-2.5 rounded-full bg-ec-loss animate-pulse" />
                                <h1 className="text-4xl font-black tracking-tighter text-[var(--color-ec-text-high)] uppercase">{ticker}</h1>
                            </div>
                            <span className="text-[10px] font-black text-[var(--color-ec-text-muted)] uppercase tracking-widest mt-1">Intraday Analysis</span>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex-1 p-6 relative">
                {loading && (
                    <div className="absolute inset-0 flex items-center justify-center bg-[var(--color-ec-bg-base)]/80 z-20 backdrop-blur-sm">
                        <div className="flex flex-col items-center gap-4">
                            <div className="w-12 h-12 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin" />
                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--color-ec-text-secondary)]">Loading Market Data</span>
                        </div>
                    </div>
                )}
                <div className="w-full h-full bg-[var(--color-ec-bg-surface)]/50 rounded-2xl border border-[var(--color-ec-border)] p-2 overflow-hidden shadow-2xl">
                    <div ref={chartContainerRef} className="w-full h-full" />
                </div>
            </div>

            {/* Footer */}
            <div className="p-4 bg-[var(--color-ec-bg-surface)] border-t border-[var(--color-ec-border)] flex items-center justify-between px-10">
                <div className="flex items-center gap-6 text-[10px] font-bold uppercase tracking-widest text-[var(--color-ec-text-muted)]">
                ...
                <div className="flex items-center gap-4">
                    <span className="text-[10px] font-black text-[var(--color-ec-text-primary)] tracking-tighter uppercase mr-4">BTT Trading Console v2.0</span>
...
const MetricItem = ({ label, value, color }: { label: string; value: string; color: string }) => (
    <div className="flex flex-col items-end">
        <span className="text-[10px] font-black text-[var(--color-ec-text-muted)] tracking-widest uppercase mb-1">{label}</span>
