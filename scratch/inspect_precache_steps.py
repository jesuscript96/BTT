...
dataset_id = "895fa968-3d15-447c-be83-411300cbcb88"
...
for y, m in ym_pairs:
    ...
    if path:
        ...
        print(f"Month {y}-{m:02d}: Path={path} | Tickers={len(tickers_month)} | Cache Exists={exists} | File={cache_key}.parquet")
    else:
        print(f"Month {y}-{m:02d}: Path=None | Tickers={len(tickers_month)}")
