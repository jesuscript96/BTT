...
### R-B-07 · DAS rechaza una orden por un motivo que NO tenemos registrado (Jaume, 23-sep; PROPUESTA)
- Situación: DAS o el bróker devuelven Send_Rej (o silencio) con un texto de motivo que no está en nuestro catálogo (el catálogo nace de la tabla C del informe de la KB: BP, not shortable, SSR, MaxLoss, Position Unreal Loss, Max Ord Cap, Symbols/Route Control, PDT, horario…). Hoy el libro solo trata el rechazo de un STOP (R-C-03: 5 intentos, aviso, cierre si no sube > 100 % en 5 min) y el rechazo por margen (guarda previa, B4).
- Detección: campo «notes» del %OrderAct Send_Rej (o timeout de B5). Comparar contra el catálogo.
- Acción: (1) **Entrada o pirámide rechazada**: no se reintenta a ciegas: un solo reintento si el motivo es transitorio conocido (BP, precio); si es desconocido, cero reintentos, la señal se pierde, aviso nivel 2 con el texto literal. Tres rechazos del mismo motivo en 10 min → se pausan las entradas de ESE ticker y se avisa. (2) **Stop rechazado**: R-C-03 tal cual, pero si el motivo es un bloqueo por símbolo o cuenta («no more trades for this symbol», MaxLoss, Symbols Control) se salta directamente a AVISO NIVEL 3 y control humano, porque reintentar no sirve; el vigilante sigue mirando el precio y, si no hay stop y el precio sube, intenta cerrar al ask (que es «unwind» y puede estar permitido). (3) **Motivo desconocido en cualquier orden**: se apunta el texto literal en el catálogo pendiente del diario, aviso nivel 3, y se congelan las ENTRADAS NUEVAS de toda la cuenta hasta /sigue (las salidas y stops siguen). (4) Cada motivo nuevo visto en demo/sombra se añade al catálogo con su tratamiento antes del canario.
- Pedir a Sage: activar «Always allow unwind positions» en la cuenta del bot (para que un bloqueo por pérdida nunca impida cerrar) y la lista de controles activos («Account Risk Params»).
- Parámetros: reintentos transitorios = 1; umbral de pausa por ticker = 3 en 10 min.
- Prueba: en demo, forzar rechazos (BP, símbolo bloqueado, precio fuera de rango) y comprobar el camino de cada uno.
- Estado: PROPUESTA (23-sep). Origen: pregunta de Jaume del 23-sep; tabla C del informe de la KB.
