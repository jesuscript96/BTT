...
| Demo | 1 semana | Mismo bot contra la cuenta DEMO de DAS: órdenes reales con dinero falso. Fills, stops residentes (principal + emergencia), cancelaciones, limpieza de R-C-11, EOD, y TODOS los simulacros del runbook (matar DAS, cortar feed, matar ejecutor/vigilante, doble instancia, Telegram caído). | Todos los simulacros superados; ninguna posición larga no deseada; reconciliación sin diferencias. **Pendiente: cómo se configura la cuenta demo en DAS/Sage [API].** |
