...
| EP-2 | Orden a mercado enviada durante un halt | **Sage (24-sep): «use the OPEN route».** Diseño: un minuto antes de la reapertura, orden a mercado por la ruta OPEN | Probar en el canario que la ruta OPEN acepta la orden durante el halt y entra en el cruce; qué mensaje devuelve el API |
| EP-3 | Dos stops de compra residentes retienen BP | **CONFIRMADO por Sage (24-sep): cada orden de compra retiene BP.** | Medir en el canario cuánto retiene cada stop (¿al límite +63 %?) con una posición mínima; con ese dato decidir: (a) dimensionar posiciones contando la retención, (b) emergencia con límite más corto, o (c) emergencia puesta por el vigilante solo cuando el principal falla (pierde la garantía de residente). Decide Jaume con el número |
