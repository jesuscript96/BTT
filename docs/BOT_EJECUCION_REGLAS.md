...
## 2f. Respuestas de Sage (24-sep, WhatsApp de Jaume)
1. **Activar el API**: se activa en el portal de Sage y, con el usuario de DAS, hay que rellenar el cuestionario del API dentro de DAS: Help > Market Data Agreement > pestaña Optional.
2. **No hay cuenta demo para el API.** Solo existe la demo gratuita de la plataforma DAS (snapshot.dastrader.com, prueba de 14 días), sin API. → La fase «demo» de R-O-03 no puede ser por API: se sustituye por un CANARIO A TAMAÑO MÍNIMO en la cuenta real (lotes de 1-100 acciones) donde se prueban fills, stops, cancelaciones, limpieza y los rechazos provocados, con coste real pequeño. La demo de la plataforma sirve solo para familiarizarse con la GUI y las rutas.
3. **«Yes, each buy order will use buying power»**: dos stops de compra vivos sobre el mismo corto retienen BP cada uno → EP-3 CONFIRMADO. Falta saber a qué precio se valora la retención (¿el límite?); se mide en el canario con una posición mínima antes de fijar tamaños.
4. **Stop durante un halt**: «Keep under the stock resume trading or if the market close»: el stop se mantiene hasta que la acción reanuda o cierra el mercado. Con TIF DAY+ el stop desaparece a las 20:00: en un halt que cruza días hay que reponerlo al día siguiente (R-C-10 al arrancar).
5. **Rebate en SAGEREBL con orden en el punto medio**: «depende del liquidity flag de la orden»: si la bolsa la marca como agregando, cobra el rebate. Y el cargo por remover es 0 solo de 07:00 a 19:00 (fuera de ese tramo, 0,003 $/acción).
6. **Orden a mercado durante un halt para entrar en el cruce de reapertura: «use the OPEN route».** → R-F-01 y EP-2: la salida durante el halt va por la ruta OPEN; probar en el canario que acepta la orden durante el halt y que participa en el cruce.

## 2g. Respuestas de DAS a las 9 preguntas neteadas (24-sep)
1. **Demo: SÍ hay forma.** «You can use the API with a training account attached to your brokerage user»; un simulador independiente exigiría pagar simulador + API, así que recomiendan la training account del propio usuario del bróker. → Jaume (24-sep): NO se contrata (cuesta bastante más y complica); la demo queda descartada y se va directo al canario mínimo con muy poco riesgo.
2. **REPLACE: sí.** Se puede modificar precio y cantidad de una orden viva sin cancelar y reenviar → R-C-11 (bajar la cantidad de la emergencia tras un fill parcial) y R-G-03 (solo bajar cantidad en cisne negro) se hacen con REPLACE, sin ventana sin stop. Sintaxis en el manual nuevo.
3. **SMAT** es el smart router y funciona a todas horas; a qué ruta manda cada orden es interno del bróker (Jaume, 24-sep: no nos ocupamos de eso; solo de si la orden se acepta o no). Captura del socio (24-sep): los stops aparecen en la ventana Orders con ruta SMAT aunque se elija otra: DAS los redirige al SMAT (coherente con el manual: «SMAT can support all stop types»). Tipo mostrado: **«SLP: 2,97 2,99»** = stop limit con disparo y límite, la «P» apunta a StopLimitP (LimitP) → RESPONDIDO: la palabra clave es STOPLMTP (ver R-C-01).
4. **PEG MID**: preguntar al bróker o enviarla al SMAT.
5. **OCO y Trigger NO están de forma nativa en el API**; sí los stops, incluido Stop Range. OCO/Trigger solo vía un script de hotkey con el comando SCRIPT. → No los usamos (el diseño ya no dependía de ellos); EP-4 se cierra.
6. **Ventanas de confirmación**: desactivarlas donde se pueda y no provocarlas. → Checklist del perfil del bot (R-O-01).
7. **Dos stops de compra vivos sobre el mismo corto: SÍ.** «They will affect your BP though in the case that both executed and put you long»: según DAS el BP se resiente si las DOS se ejecutan y la cuenta queda larga; Sage dijo que cada orden de compra retiene BP. Contradicción aparente → EP-3 se mide en la training account: BP antes y después de poner los dos stops.
8. **Órdenes durante un halt**: «Most routes will not accept orders on halted symbols», es cosa del bróker. Sage dijo ruta OPEN. → EP-2: probar en el canario la ruta OPEN con un símbolo en halt.
9. **Post only: PO SÍ está soportado; ALO no.** Sintaxis en el manual. → R-B-01: la orden del punto medio va con PO: si al llegar fuera a remover, la bolsa la rechaza o recoloca en vez de ejecutarla; el redondeo hacia arriba al tick se mantiene como colchón.
...
**DEFINICIÓN DEFINITIVA de los márgenes (Jaume, 23-sep): SUMANDO sobre el nivel del stop de la estrategia.** El nivel de stop lo pone la estrategia (Previous Max, +10 %, ATR…). Sobre ese nivel: principal LimitP con disparo EN el nivel y límite a **+3 %**; margen de **+10 %**; emergencia LimitP con disparo a **+13 %** y límite a **+63 %**. Todo son distancias sobre el nivel del stop, no sobre la entrada. (Antes el libro las multiplicaba, 1,03 × 1,10 × 1,50 = +70 %, y el «+87 %» era lo mismo medido desde la entrada con el stop a +10 %; esa forma queda anulada.) Estudio de fogonazos repetido con +3 / +13 / +63 (`40c_fogonazos_margenes_suma.py`, 251 casos): idéntico en la práctica: 10 k → subida 96 %, sin cerrar 0 %, pérdida sobre la cuenta media 2,6 %, p90 6,2 %, máx 7,7 % (antes 8,4 %: el techo más corto recorta el peor caso). EP-5 CERRADO.

**CONFIRMADO por DAS (24-sep): el LimitP existe en el API como palabra clave `STOPLMTP` en NEWORDER** («you need to send a Stop LimitP order, which in NEWORDER is STOPLMTP. This is in the manual»). Definición de DAS: «Stop LimitP is just a Stop Limit order that can trigger in pre/post market (as well as regular hours). Other Stop orders do not trigger in pre/post, only regular hours.» → Principal y emergencia se envían como STOPLMTP con disparo y límite (+3 % y +13 %/+63 % sobre el nivel del stop). El detalle de por qué precio dispara (último print dentro del spread, según la KB) se comprueba en el canario. Pregunta cerrada.
...
- Aclaración (Jaume, 24-sep): el protocolo y el veto de reentrada solo se activan si el precio PASA DE LARGO la emergencia y quedan acciones al descubierto. Si la emergencia (o el principal) cierra toda la posición, NO hay protocolo: el ticker sigue operable y una señal nueva se toma según la estrategia (R-D-04), con locate nuevo si el anterior se consumió.
- Estado: FIJADA (Jaume, 20-sep; aclarada 24-sep). Origen: pregunta 2 del repaso de stops; reentrada tras BS.
...
Ninguna IPO ni SPAC tuvo cisne negro salvo PLYX (1B, 15 días cotizando, fogonazo +2.247 %, salió por el SL al −32 %). Lectura: excluir las SPAC está bien en las dos (pierden). Excluir las IPO < 30 días en 1B QUITA sus mejores trades (+1.900 % de retorno sumado en 177 trades); en 2B son neutras en rentabilidad pero doblan los halts. **FIJADO por Jaume (24-sep): SPAC fuera en todas; IPO < 30 días fuera solo en las estrategias de RTH, no en las de PM.**
**Riesgo de T1/T12 y de fogonazo por tipo (24-sep, `48_ipo_spac_t12_fogonazos.py`, 47 T12/T1 sin reabrir de 2019-2026 y 278 fogonazos):** SPAC = **7 de los 27 T12 (26 %)** con solo ~1 % de presencia en los trades → fuera, confirmado. IPO < 30 días: **0 de 27 T12 y 0 de 20 T1**; ninguna con menos de 160 días cotizando (mediana 815 días en T12, 1.715 en T1). En fogonazos las IPO < 30 días son el 6 % de los ticker-día (frente al ~4 % de los trades de 1B): algo más de fogonazo (PLYX +2.247 %, FSSI +488 %), nada de T12. Conclusión: las IPO recientes tienen riesgo de fogonazo algo mayor (que cubre el protocolo de stops) y NINGÚN riesgo medido de T12/T1; en premercado (1B) son las más rentables y en sesión (2B) doblan los halts LULD sin aportar rentabilidad.
...
- Acción (v2, FIJADA por Jaume el 24-sep): lista NEGRA manual en el cuadro de mandos (Jaume mete tickers a mano, p. ej. OPAs conocidas) + exclusiones automáticas del radar: **SPAC (SIC 6770) fuera SIEMPRE, en todas las estrategias; IPO < 30 días fuera SOLO en las estrategias de sesión regular (RTH, p. ej. 2B), NO en las de premercado (1B): en PM son las más rentables y no tienen riesgo medido de T12/T1;** split/contrasplit del día fuera; OPA por banda clavada = aviso, decide el humano. La exclusión de IPO es una casilla por estrategia en el cuadro (encendida por defecto en las de RTH, apagada en las de PM).
...
| EP-2 | Orden a mercado enviada durante un halt | **Sage: «use the OPEN route». DAS: «most routes will not accept orders on halted symbols».** Diseño: un minuto antes, a mercado por OPEN | Probar en el canario que OPEN acepta la orden con el símbolo parado y entra en el cruce; qué devuelve el API; si no, enviar al reabrir |
| EP-3 | Dos stops de compra residentes retienen BP | **Sage (24-sep): cada orden de compra retiene BP. DAS (24-sep): el BP se resiente si las dos se ejecutan y quedas largo. Contradicción aparente.** | Medir en el canario cuánto retiene cada stop (¿al límite +63 %?) con una posición mínima; con ese dato decidir: (a) dimensionar posiciones contando la retención, (b) emergencia con límite más corto, o (c) emergencia puesta por el vigilante solo cuando el principal falla (pierde la garantía de residente). Decide Jaume con el número |
| EP-4 | Trigger Order del API | **CERRADO (DAS, 24-sep): el API no tiene OCO ni Trigger nativos; el bot pone los stops él mismo tras cada fill, como estaba** | — |
...
## 6. Huecos detectados el 24-sep (repaso de arquitectura) — revisados con Jaume el mismo día
**Decisión de Jaume:** H-1, H-3, H-7 y H-8 fuera (H-1 se deduce del diario; nunca > 50 candidatas; los locates siempre ≥ 100; grupo de Telegram privado, el filtro por chat id queda como detalle de implementación de una línea). H-2 = el DIARIO: registro de todo (órdenes, locates, errores, avisos) que además es la materia prima de Telegram, y que el bot relee al arrancar para no repetir nada. H-4, H-5 y H-6 quedan como detalles de implementación (fichero atómico; un ticker con datos imposibles no para el bucle; hash del motor comprobado al arrancar y el bot no opera si no coincide).
| Id | Hueco | Riesgo si no se cubre | Propuesta |
|---|---|---|---|
| H-1 | **Datos de partida malos** (cierre de ayer erróneo o ausente, calendario, universo): ya pasó en el bot de alertas (cierre de 3 sesiones atrás, universo recortado) | El radar calcula gaps falsos → señales falsas o ausentes, sin error | Puerta de calidad al arrancar: cierre de ayer por dos fuentes (Massive REST y lago) y, si difieren > 1 % o falta, ese ticker NO se opera y se avisa; calendario comprobado contra Massive; aviso si el universo baja del 80 % del habitual |
| H-2 | **Estado persistido tras un reinicio**: qué señales ya se actuaron, qué locates se compraron y usaron, qué pausas por ticker hay | Tras relanzar, el bot podría reentrar en una señal ya operada o comprar locates dos veces | Diario en disco escrito ANTES de enviar cada orden (write-ahead); al arrancar se reconstruye el estado desde el diario + DAS (R-C-10), nunca desde cero |
| H-3 | **Radar con más de 50 candidatas**: qué 50 entran en DAS | Sin criterio, entran las primeras que lleguen y se pierden las mejores | Prioridad por cercanía a la señal (las que ya cumplen condición previa de alguna estrategia), luego por gap y dólares negociados; recálculo cada minuto; el resto sigue en Massive sin precios de DAS |
| H-4 | **Fichero de configuración del cuadro** escrito a medias o corrupto | El bot arranca con tamaños vacíos o viejos sin saberlo | Escritura atómica (fichero temporal + renombrado), versión y hash dentro; el bot valida el hash y, si falla, usa el último bueno y avisa |
| H-5 | **Un error en un ticker tumba el bucle entero** (dato raro, excepción en un indicador) | Un ticker malo deja sin vigilancia a los demás | Cada ticker se evalúa aislado (try/except por ticker); un fallo se registra, se pausa ese ticker y el bucle sigue; aviso nivel 2 |
| H-6 | **Versión del motor**: el backtester cambia un indicador y el bot sigue con el viejo (o al revés) | Señales distintas en bot y backtest sin ningún error | La exportación de estrategias lleva el hash del motor; el bot compara al arrancar y se niega a operar si no coincide (R-O-02) |
| H-7 | **Lotes sueltos (< 100 acciones)** en premercado | Las órdenes de menos de 100 no aparecen en la cotización NBBO; agregando en el punto medio puede que nadie las vea | Medir en sombra/canario el % de fill de lotes sueltos; si es malo, redondear a 100 cuando la posición lo permita (encaja con los paquetes de locates) |
| H-8 | **Comandos de Telegram desde un chat ajeno** | Cualquiera con el enlace podría mandar /cerrar todo | Solo se aceptan comandos del chat id de Jaume (y del socio para los que se decidan); el resto se ignora y se registra |
