...
1. **Demo: SÍ hay forma.** «You can use the API with a training account attached to your brokerage user»; un simulador independiente exigiría pagar simulador + API, así que recomiendan la training account del propio usuario del bróker. → La fase DEMO vuelve (R-O-03): pedir a Sage una training account ligada al usuario de Jaume; el canario mínimo se mantiene después.
...
3. **SMAT** es el smart router y funciona a todas horas; a qué ruta manda cada orden lo decide el bróker (preguntar a Sage). Captura del socio (24-sep): los stops aparecen en la ventana Orders con ruta SMAT aunque se elija otra: DAS los redirige al SMAT (coherente con el manual: «SMAT can support all stop types»). Tipo mostrado: **«SLP: 2,97 2,99»** = stop limit con disparo y límite, la «P» apunta a StopLimitP (LimitP) → PREGUNTA a DAS: ¿el STOPLMT del API crea esa misma orden SLP (disparo por último precio) o hay palabra clave distinta?
...
8. **Órdenes durante un halt**: «Most routes will not accept orders on halted symbols», es cosa del bróker. Sage dijo ruta OPEN. → EP-2: probar en la training account la ruta OPEN con un símbolo en halt.
...
| EP-2 | Orden a mercado enviada durante un halt | **Sage: «use the OPEN route». DAS: «most routes will not accept orders on halted symbols».** Diseño: un minuto antes, a mercado por OPEN | Probar en la training account que OPEN acepta la orden con el símbolo parado y entra en el cruce; qué devuelve el API; si no, enviar al reabrir |
