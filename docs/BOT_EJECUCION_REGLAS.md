...
3. **SMAT** es el smart router y funciona a todas horas; a qué ruta manda cada orden es interno del bróker (Jaume, 24-sep: no nos ocupamos de eso; solo de si la orden se acepta o no). Captura del socio (24-sep): los stops aparecen en la ventana Orders con ruta SMAT aunque se elija otra: DAS los redirige al SMAT (coherente con el manual: «SMAT can support all stop types»). Tipo mostrado: **«SLP: 2,97 2,99»** = stop limit con disparo y límite, la «P» apunta a StopLimitP (LimitP) → PREGUNTA a DAS: ¿el STOPLMT del API crea esa misma orden SLP (disparo por último precio) o hay palabra clave distinta?
