...
| P17 | **Dos documentos para el socio (Jaume, 21-sep):** (a) borrador de arquitectura (21-sep) SUSTITUIDO el 22-sep por la GUÍA explicada para el socio (`bot_ejecucion/guia_socio_pdf.py` → Guia_bot_ejecucion_socio_2026-09-22.pdf, 5 págs, cada bloque con su «por qué»); (b) versión FINAL cuando esté todo (PDF del bróker, API, VPS) con lo que haya cambiado. RECORDÁRSELO al cerrar la fase de código. | (a) hecho, (b) pendiente |
