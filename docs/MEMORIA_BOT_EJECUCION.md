...
| P13 | **Fogonazos por tamaño** (`40_fogonazos_por_tamano.py`, 20-sep): 39 fogonazos con libro → 0 % colgados a 30 min; salida en la subida 92 % (10 k) → 74 % (500 k); pérdida sobre la cuenta con 10 % por posición: media 3 %, p90 6 %, máx 7,9 % (ENSC). Tabla en el libro (área G). **Jaume la preguntará en el futuro: recordársela.** En marcha: descarga del libro de los otros 255 fogonazos (`41_…`, tope 9 $) para repetir con más muestra y AÑADIR la sección al informe de fogonazos (parche v11 de `12_informe_pdf.py`) | Descarga en curso |
