...
### [DECISIÓN + FEATURE · 2026-09-10 · COMPARTIDAS] La pestaña de Álvaro se queda, pero como VISOR: aquí no se importa nada (Jaume)
- **Lo que decidió Jaume:** «solo va a ser para compartir la visualización del JSON de la estrategia, para ver los stats de la misma, y ya si uno quiere que se haga la suya en función de dichos stats». Es decir: se comparte la RADIOGRAFÍA para poder replicarla a mano, no la estrategia para cargarla.
- **Por qué es la decisión correcta (dos razones técnicas, no de gusto):** (1) el JSON lleva el `dataset_id` del otro dev, que no existe en tu base; (2) cualquier campo de la rama de Álvaro que la nuestra no conozca **se caería en silencio** al importar ([[listas blancas en tres capas]]). Un visor no puede romper nada.
- **NO se borró nada de Álvaro, a propósito y por una razón práctica:** su formato de fichero, su carpeta `estrategias_compartidas/<dev>/` y su backend son justo lo que permite que se lean mutuamente. Cambiar el formato habría roto el intercambio, que es el objetivo. Se hizo **merge de `origin/staging`** (conflicto solo en la memoria madre, resuelto quedándose las dos versiones; `ResultsTabs.tsx` fusionó solo y conviven su pestaña «Compartidas» y nuestra sub-pestaña «Banda de locates»). Lo único reescrito es su `SharedStrategiesTab.tsx`.
- **Qué muestra al desplegar una estrategia:** cifras de forma (sesgo, día, nº de condiciones de entrada y salida, niveles de pirámide, filtros de universo), cuándo puede entrar (sesiones, horario, ventanas, temporalidad), universo con sus reglas, **árbol legible de condiciones** de entrada y salida (con los grupos anidados y su Y/Ó), precondiciones post-gap, riesgo y salidas (stop, tamaño por stop, Cangrejo, TP, parciales, trailing, swing), pirámides nivel a nivel, y el JSON crudo con botón de copiar.
- **REQUISITO DE JAUME: «que estén bien especificados».** Traducido a código: **ninguna clave del fichero puede quedar invisible**, que es la misma trampa de las listas blancas pero en la pantalla. Cada sección pinta además las claves que NO desglosa a mano; lo que no encaja en ninguna cae en «Otros ajustes»; y dentro de cada condición, del stop, del trailing, del swing, de los parciales y de cada nivel de pirámide, los campos no reconocidos se añaden detrás en vez de perderse. `dataset_id` e `is_wizard` salen también, en «Otros ajustes».
- **Setup:** `SHARED_STRATEGIES_OWNER=sailor` añadido a `backend/.env` (no se commitea); sin eso las compartidas de Jaume caerían en `dev/`. El endpoint es nuevo, así que **exige reiniciar el backend** o da 404 (la pestaña lo dice en el error).
...
- **Las dos trampas, medidas en banco ANTES de escribir el módulo:** mandar la ráfaga de 5.700 velas una a una cuesta 84 ms al bot y en un solo envío 3,4 ms (siempre en lote); y el lote pesa más que el buffer de la tubería, así que `send` se bloquearía si el hijo estuviera barriendo — justo lo que se venía a evitar. De ahí: hilo lector en el hijo (9 ms por ráfaga) y de envío en el padre, con cola acotada que tira lo viejo antes que esperar.
- **Si el hijo muere:** el bot lo levanta y le devuelve su propio estado del mercado (`MercadoEnVivo.volcar()`/`cargar()`, nuevos), para no perder el máximo de premercado del día y dejar de admitir tickers que ya cumplían. Probado matándolo en un test.
- **Las alertas vuelven a la vela OFICIAL.** Las velas propias del 21-sep se apagan (`BOT_VELAS_PROPIAS`, por defecto 0). Se montaron para no esperar «3 s», pero esos 3 s eran del canal por SEGUNDO (`A.`): la métrica `[LATENCIA]` medía ese canal, no la vela de minuto, y nadie medía la de minuto. La vela oficial sale del socket a **1,49 s**; la propia se cerraba a 1,0 s y a cambio **el 43 % de las velas tenía el cierre distinto del oficial** (0,43 % de mediana, hasta 5,7 %), porque los dark pools del propio minuto se publican hasta 6 s tarde. La prealerta se queda con los prints (0,26 s), que es lo que sí se ganó el 21-sep.
- **Métricas nuevas en el log del bot:** `[LATENCIA vela de minuto]` (lo que tarda la ALERTA desde que cierra el minuto) y `[LATENCIA socket]` (retraso de la vela al salir de `recv` + mayor rato sin leer del socket). Son las que permitieron encontrar todo esto; la vieja `[LATENCIA]` pasa a llamarse `[LATENCIA canal por segundo]` para no volver a confundirlas.
- **Coste real de la latencia:** 2 s equivalen a que el precio se mueva 0,42 % de mediana (p90 0,87 %), pero con signo la mediana es **0,00 %**: es ruido, no coste esperado. De las 49 alertas del día, todas cayeron en velas de 1,2-2,3 s.
...
### [HALLAZGO · 2026-09-25 · 07] `POST /api/data/filter` devuelve 500 («Out of range float values are not JSON compliant: nan») en cuanto la selección contiene un NULL numérico — el buscador revienta en esos filtros
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** bug
- **Dónde:** `backend/app/routers/data.py:297-322` (`df.to_dict(orient="records")` del resultado completo → el serializador JSON de FastAPI rechaza floats NaN)
- **Qué observé:** cualquier búsqueda cuyo resultado tenga filas con columnas float a NULL (p. ej. `vol_rel_20` en tickers con <20 días de historial, `shares_outstanding`/`rotacion_dia` sin parquet de circulación) responde 500 en vez de datos. En el universo PMH Gap ≥ 50 (microcaps jóvenes) pasa SIEMPRE, con cualquier rango de fechas probado (2019-2026).
- **Cómo reproducir:** `curl -s -X POST http://127.0.0.1:8010/api/data/filter -H "Content-Type: application/json" -d '{"date_from":"2025-01-01","date_to":"2025-01-31","rules":[{"id":"r1","category":"gap","metric":"PMH Gap %","operator":">=","valueType":"static","value":"50"}]}'` → 500 `{"detail":"Internal Server Error","message":"Out of range float values are not JSON compliant: nan"}`.
- **Evidencia:** 8/8 requests (2019-2026) con ese mismo 500 durante la sesión del Bloque 1 (`.tmp_bloque1/paso1_universo.py`). Busquedas de rangos sin NULL pasan: el fallo depende de las filas devueltas, no de la petición.
- **Hipótesis de causa:** HIPÓTESIS — las columnas calculadas del endpoint (`vol_rel_20`, `pm_vol_rel_20`, `vol_prev3_rel_20`) y las opcionales de circulación dejan NaN que `to_dict` no convierte a `null`.
- **Impacto:** la página del buscador que usa este endpoint falla (spinner/error) en cualquier filtro que capture tickers jóvenes — justo el universo de gappers. Además bloqueó la vía API para extracciones de análisis (tuve que usar el ciclo read-only autorizado).
- **Código tocado:** NINGUNO (confirmado)
- **Estado:** ABIERTO

### [HALLAZGO · 2026-09-25 · 08] `require_shortable` y `exclude_dilution` NO filtran nada al materializar los pares de un dataset (se descartan por ser bool) — el universo 1B real es solo PMH Gap ≥ 50 + tipo + split + precio
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** inconsistencia
- **Dónde:** `backend/app/services/query_service.py:242-249` (`if isinstance(v, bool): continue`, con comentario que lo asume a propósito para evitar un BinderException), vía `_compute_dataset_pairs` (`backend/app/routers/query.py:194-234`)
- **Qué observé:** los interruptores booleanos del panel de creación de datasets (`require_shortable: true`, `exclude_dilution: true`) no existen como columnas de `daily_metrics` y el constructor de la query los SALTA en silencio. El PRD 1B y la UI anuncian «Universo: PMH Gap % ≥ 50, shortable, excluye dilución»; los pares materializados SOLO aplican: regla PMH, tipo CS/ADRC/OS (vía `tickers`), sin split ese día y `_filtrar_universo` (mapa de tipos + suelo 0,10 $).
- **Cómo reproducir:** comparar datasets gemelos con y sin flags: `d088a358` (2024-01-01→2025-01-01, PMH≥50, `require_shortable`+`exclude_dilution` = true) → 4.200 pares; `e2d1eb51` (mismo rango y regla, SIN flags) → 4.200 pares. Idénticos (`.tmp_estudio_1b/datasets.json`).
- **Evidencia:** los dos pair_count de arriba; código citado (el propio comentario del commit explica que el bool «se colaba» generando `require_shortable >= 1.0` y lo saltaron en vez de materializar el filtro).
- **Hipótesis de causa:** no hay tabla shortable/dilución consultable en la vía local de `daily_metrics`; el filtro quedó prometido en la UI sin respaldo de datos y se desactivó para no tumbar la query.
- **Impacto:** cualquier análisis o backtest que asuma que el universo 1B excluye no-shorteables o diluyentes está asumiendo algo que no ocurre (locates/sim pueden compensar en runtime, pero el DATASET no filtra). Para el Bloque 1 de la investigación de criterios, el universo base efectivo es PMH≥50+tipo+split+precio (así lo repliqué).
- **Código tocado:** NINGUNO (confirmado)
- **Estado:** ABIERTO

### [HALLAZGO · 2026-09-25 · 09] Drift de `pair_count` de datasets antiguos vs la misma regla sobre la BD actual: `d088a358` (2024) anuncia 4.200 pares; hoy la regla da 3.821 (−9 %)
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** duda
- **Dónde:** pares materializados de `d088a358-8c31-4958-a2b0-a6df0693bbe9` (creado 2026-08-16) vs `daily_metrics` actual (`fidelidad.csv` de `.tmp_bloque1/`)
- **Qué observé:** con la MISMA regla (PMH Gap % ≥ 50, sin filtros extra) por año: 2024 hoy = 3.821 filas, pero el dataset de 2024 guardado anuncia 4.200 (−9,0 %). Los construidos más recientemente sí cuadran: 2022 → 1.157 vs 1.158 hoy; 2025 (`c8bcddc7`, 31-ago) 3.103 vs 3.098 (pre-filtro final); 2026 a 4-sep (`180d7b20`, 7-sep) 2.072 vs ~2.070 equivalentes. El par (regla viva vs dataset congelado) solo diverge fuerte en el dataset más viejo.
- **Cómo reproducir:** `GET /api/data/datasets` (pair_count por dataset) vs `fidelidad.csv` del Bloque 1 (contadores por año de la regla sobre la BD del 25-sep-2026, backend corriendo con `DISABLE_GCS_SYNC=true`).
- **Evidencia:** `.tmp_bloque1/fidelidad.csv` + `.tmp_estudio_1b/datasets.json`. En el informe del rango/neto de esta mañana ya asomaba: `e5ca2514` (2025+2026, 21-ago) = 7.143 pares, incompatible con 3.103+~2.000 de los construidos días después.
- **Hipótesis de causa:** HIPÓTESIS — el lago/BD se repuebla y depura (duplicados, revisions) y los pares de un dataset quedan congelados en el estado del día en que se materializaron; no hay re-materialización automática.
- **Impacto:** comparar `pair_count` de datasets entre sí o con recuentos actuales puede llevar a conclusiones erróneas (p. ej. «2024 tenía muchos más gappers»). Para análisis por año es más seguro recomputar la regla sobre la BD del día (como hice en el Bloque 1). Un backtest sobre un dataset viejo opera exactamente sus pares congelados, no los que la regla daría hoy.
- **Código tocado:** NINGUNO (confirmado)
- **Estado:** ABIERTO

### [HALLAZGO · 2026-09-25 · 10] `% Fade (previous_max, ap.RTH)` es NaN en TODA vela de una sesión 04:00-08:45 → condición siempre falsa en silencio: el «OR» de pirámide de la 1B-Sobri3-B no puede disparar nunca y las stats no se mueven
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** duda
- **Dónde:** `backend/app/services/indicators.py:1494-1510` (`_ap_session_started`: `ap.RTH` enciende la máscara solo a partir de las 09:30) + `indicators.py:2441-2456` (`% Fade` = `(ref−close)/ref` con `ref` = cummax enmascarado → NaN si ninguna vela del frame pasa la máscara). Estrategias: `5ed17d89` (1B-Sobri3) y `abfd6283` (1B-Sobri3-B, creada hoy 13:42).
- **Qué observé:** Álvaro cambió la pirámide de la 3 (grupo único AND·3 %) a dos ramas con OR (grupo 1 = VWAP-cerca ∧ Elapsed≥20 ∧ Close>EMA10; grupo 2 = `%Fade>15` ∧ Elapsed<15 ∧ VWAP-cerca) + 4 % de añadido, y las stats «no cambiaban». Verificado con el motor real: (a) la definición guardada de la 3B llega íntegra al motor (OR y grupos anidados persisten y compilan); (b) 3B-OR·4 % es BIT-IDÉNTICA a la 3-original·con-4 % (ret 16007,3887 % · DD −13,8557 % · 1831 trades · 1.065 añadidos, mismos ticker-días y mismas horas de añadido); (c) la rama 2 no aporta NADA porque `% Fade (previous_max, ap.RTH)` es NaN en el 100 % de las velas del frame de la sesión custom 04:00-08:45 (la máscara `ap.RTH` solo se activa ≥09:30 y en el frame no existe ninguna) → `%Fade > 15` es falsa en todas las velas, sin warning alguno. En el frame día-completo del parquet sí tiene valores (las velas ≥16:00 del día anterior encienden la máscara), lo que hace el problema especialmente difícil de cazar a ojo.
- **Cómo reproducir:** (1) backtests IS 2025 con definición inline: `5ed17d89` tal cual → 10194,9995 %; misma con `capital_pct:4` → 16007,3887 %; `abfd6283` tal cual → 16007,3887 % (idéntica); `abfd6283` con root `AND` → 2515,99 % con 0 añadidos (ramas mutuamente excluyentes por Elapsed <15 vs ≥20). Peticiones exactas en `.tmp_estudio_1b/corridas.jsonl` (tags SO3-AND-IS, SO3B-OR-IS, SO3B-AND4-IS, SO3-PCT4-IS). (2) Bar-level: `.tmp_estudio_1b/diag_frame_sim.py` — frame 04:00-08:45, `%Fade NaN 286/286` (AACG 2025-08-04) etc., 0 velas True de la rama 2; en frame día completo (`.tmp_estudio_1b/diag_rama2_timing.py`) la rama 2 sí tiene velas True, casi todas ≥09:30 o previas a la entrada.
- **Evidencia:** corridas auto-guardadas de hoy (`GET /api/strategy-search/recent`): 12:11:26 def. original → 10195,0 % · −12,98; 10:45, 13:41:23 y 13:42:34 con la config nueva → 16007,4 % · −13,86 tres veces IDÉNTICAS (el toggle del OR no mueve nada); guardado de la 3B a las 13:42:54. Resultados crudos: `.tmp_estudio_1b/resultados/SO3*.json`.
- **Hipótesis de causa:** HIPÓTESIS — no es bug de evaluación: la semántica documentada de `ap.RTH` (arrancar el acumulado en la apertura RTH) es honesta con NaN antes de las 09:30. El problema es de diseño/UX: una condición estructuralmente imposible en la sesión de la estrategia se compila y corre en silencio (a diferencia del caso 1d, que sí loguea error en `strategy_engine.py:1964`), y la UI ofrece `ap.RTH` para `% Fade` sin avisar de que en sesiones premarket es siempre NaN.
- **Impacto:** cualquier condición con `% Fade`/`Previous max`/`Previous min` + `ap.RTH` (o `ap.AM` ≥16:00) dentro de una sesión que termina antes de esa hora es una rama muerta silenciosa — el backtest corre igual y el usuario no ve ni warning ni efecto. La rama «early fade» de la 3B nunca existió de facto; la única diferencia real de la 3B vs la 3 es el 3 %→4 % del añadido. Para un fade intra-premarket la referencia utilizable es `ap.PM` (acumula desde la primera vela del frame) o `fade_ref: vwap_cross`.
- **Código tocado:** NINGUNO (confirmado)
- **Estado:** ABIERTO

### [HALLAZGO · 2026-09-25 · 11] Creación de datasets ROTA en el backend local: «Binder Error: infinite recursion detected: attempting to recursively bind view "tickers"» al materializar pares (los backtests sobre datasets ya materializados funcionan)
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** bug (bloquea crear universos nuevos; no afecta a corridas sobre datasets existentes)
- **Dónde:** `backend/app/routers/query.py` → `_populate_dataset_pairs` → `build_screener_query` (join a `massive.tickers`); el mismo error aparece como WARN en `_init_connection_views` («Failed to setup massive views on connection») y en la inicialización de market data views («Could not initialize market data views»)
- **Qué observé:** al crear un dataset por el flujo normal (`POST /api/queries/`, reglas PMH Gap ≥ 50 + Premarket Volume ≥ 2M, 2024), el registro va bien pero el trabajo en background muere al instante con la recursión de vista y el dataset queda con 0 pares (status `error` en `precache-status`). NO es exclusivo de mi intento: otro dataset (`6712f8d4`, de esta mañana) lleva fallando igual todo el día con reintentos cada pocos minutos (parece la UI). En cambio, los backtests sobre datasets YA materializados corren perfectos (uno de 740 trades terminó justo antes de mi intento).
- **Cómo reproducir:** `POST /api/queries/` con cualquier regla → `GET /api/queries/precache-status/{id}` → `error`, 0 pares; en `backend_prof.log`: «[ERROR] Background dataset creation failed … Binder Error: infinite recursion detected: attempting to recursively bind view "tickers"» (líneas 24157 y 50771 de hoy).
- **Evidencia:** `backend_prof.log` (24157 para `6712f8d4` por la mañana, 50771 para mi `a9257423` a las 13:2x) + WARN de massive views desde el arranque del log. Mi extracción read-only de esta mañana leyó `tickers`/`splits` como tablas sin problema → el ciclo de vistas vive en el catalog que ve el backend, no en una tabla rota.
- **Hipótesis de causa:** HIPÓTESIS — ciclo persistido entre vistas (`main.tickers` ↔ `massive.tickers`), coherente con `_init_connection_views` creando `massive.tickers AS SELECT * FROM main.tickers` cuando `main.tickers` sería a su vez vista sobre `massive.tickers` (p. ej. tras una sesión en modo GCS, que crea `tickers AS SELECT * FROM massive.tickers` persistente). Confirmarlo exige inspeccionar el catalog con el backend parado.
- **Impacto:** no se pueden crear universos nuevos ni añadir pares a existentes desde local mientras dure. El dataset vacío de mi intento (`a9257423`) lo he borrado con `DELETE /api/queries/{id}`. Workaround usado en el punto 5 del Bloque 1: dataset 2024 ya existente sin la regla de volumen del universo DT, verificada equivalente (mismas trades exactas que con la regla).
- **Código tocado:** NINGUNO (confirmado). Borrado solo de MI dataset vacío a9257423 (creado por mí minutos antes, 0 pares).
- **Estado:** ABIERTO

### [HALLAZGO · 2026-09-25 · 12] ACTUALIZACIÓN del 11 — causa raíz CONFIRMADA: no hay vistas persistidas (catálogo limpio); el ciclo lo crea EN RUNTIME la rama local de `_init_connection_views`/`init_db` al rebindar `main.X` dentro de la db attachada `massive` (vistas con nombre 2-part). Funciona en duckdb 1.1.3 (la pineada), recurse en 1.5.5 (la del venv desde el 7-sep). Los renames del WAL no tienen nada que ver
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** bug (confirmado y reproducido; fix propuesto, NO aplicado)
- **Dónde:** `backend/app/database.py:46-55` y `backend/app/init_db.py:119-137` (ramas `provider == "local"`): `ATTACH ':memory:' AS massive` + `CREATE VIEW [IF NOT EXISTS] massive.<t> AS SELECT * FROM main.<t>`
- **Qué observé (diagnóstico con el ciclo read-only autorizado):**
  1. **Catálogo de `local_data.duckdb` LIMPIO**: `tickers`/`splits`/`daily_metrics`/`intraday_1m` son TABLAS reales de `main` (36.554/27.239/19,35M/3.000M filas); NO existe el esquema `massive` ni vista alguna en el fichero (`.tmp_bloque1/catalogo_diag.txt`). Mi hipótesis original del ciclo persistido queda DESCARTADA.
  2. **Reproducción aislada** (`.tmp_bloque1/paso9_repro.py`, fichero scratch, secuencia literal del código): con duckdb **1.5.5** del venv, `CREATE VIEW massive.tickers AS SELECT * FROM main.tickers` se crea pero al CONSULTARLA da `Binder Error: infinite recursion detected: attempting to recursively bind view "tickers"` — el cuerpo `main.tickers` se re-resuelve DENTRO del catálogo de la vista (la db attachada `massive`) → la vista se referencia a sí misma. Con duckdb **1.1.3** (la pineada en requirements), la MISMA secuencia funciona en todos los escenarios (misma conexión, segunda conexión, tercera sin re-init).
  3. **Cuándo se rompió**: el venv tiene duckdb 1.5.5 desde el **7-sep 15:10** (mtimes de site-packages; requirements.txt pide 1.1.3). El log del backend (nace el 27-ago, print de éxito «Saved combinations for dataset» existe desde el 26-feb) tiene **CERO materializaciones exitosas por la vía POST /api/queries en toda su vida** → la vía POST lleva rota AL MENOS desde el 27-ago y pasó desapercibida porque los pares llegan igual por la vía del orquestador (`fetch_qualifying_data` + INSERT dinámico al primer backtest).
- **Corrección de tiempos de mi entrada 11**: `6712f8d4` se creó HOY a las **17:13** (created_at; su primer fallo es de entonces) y mi intento `a9257423` fue ~17:2x — AMBOS horas después de los renames de WAL (11:47/11:59 y 13:12). **Los renames del WAL no tienen nada que ver.** (En la entrada 11 escribí «por la mañana» y «13:2x»: incorrecto.)
- **Cómo reproducir:** `backend/.venv/Scripts/python.exe .tmp_bloque1/paso9_repro.py` (recursa con 1.5.5) · `PYTHONPATH=.tmp_bloque1/ddb113 backend/.venv/Scripts/python.exe .tmp_bloque1/paso9_repro.py` (pasa con 1.1.3 instalada ahí).
- **Evidencia:** `catalogo_diag.txt` (catálogo limpio) · `paso9_repro.py` (repro dual) · `backend_prof.log` 0×«Saved combinations» · venv `duckdb-1.5.5.dist-info` mtime 7-sep 15:10 · requirements `duckdb==1.1.3`.
- **Fix PROPUESTO (validado en 1.5.5 y 1.1.3, NO aplicado — espera OK de Álvaro):** en las DOS ramas locales (database.py y init_db.py), cualificar con nombre 3-part dinámico: `cur_db = con.execute("SELECT current_database()").fetchone()[0]` → `CREATE VIEW [IF NOT EXISTS] massive.<t> AS SELECT * FROM {cur_db}.main.<t>`. Validado en scratch con ambas conexiones (.tmp_bloque1/scratch_fix). Alternativa descartada de momento: bajar duckdb a 1.1.3 (revierte la 1.5.5 que alguien instaló a propósito el 7-sep).
- **Impacto:** es un fallo de CÓDIGO (no de la BD local): le pasa a cualquier entorno `DB_PROVIDER=local` con duckdb moderno (Sailor incluido). Producción (gcs/motherduck) usa otras ramas con parquet/3-part y no está afectada. Mientras no se arregle: crear datasets por la UI/API deja el dataset con 0 pares hasta que alguien corre un backtest encima (que los materializa) — silencioso y confuso.
- **Código tocado:** NINGUNO (confirmado; scratch y .tmp solo).
- **Estado:** ABIERTO (fix propuesto, esperando OK)

### [HALLAZGO · 2026-09-25 · 13] RESUELTO 11/12 — fix aplicado con el OK de Álvaro: vistas massive.* con nombre 3-part dinámico (commit `cd180fa`, rama `alvaro-rama-desarrollo`, SIN push). Aviso para Sailor + hipótesis WAL/duckdb
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** (cierre de 11 y 12)
- **Qué se aplicó:** en `backend/app/database.py` (`_init_connection_views`, rama local) y `backend/app/init_db.py` (rama local), las 4 vistas `massive.<t>` pasan de `SELECT * FROM main.<t>` a `SELECT * FROM {current_database()}.main.<t>` (nombre 3-part dinámico). Copia de seguridad de ambos ficheros ANTES de tocar: `.tmp_bloque1/backup_hallazgo11/`. Commit `cd180fa` en `alvaro-rama-desarrollo`, **sin push** (pendiente OK de Álvaro).
- **Verificación completa (25-sep, ~18:1x):** (1) rearranque vía ciclo taskkill+watchdog autorizado (WAL de 160 B renombrado a `.stalled_fix11_1810`, nunca borrado); (2) log de arranque LIMPIO: «[INFO] Local market data views virtualized in massive schema» y ninguna aparición de «infinite recursion» ni «Could not initialize market data views» en el arranque nuevo; (3) prueba end-to-end: `POST /api/queries/` con dataset pequeño (PMH Gap ≥ 50, junio-2025) materializó **303 pares en ~5 s por la vía POST** (antes moría al instante con 0 pares); (4) dataset de prueba borrado (`TEST_FIX_hallazgo11_borrar`, `f88132e7`); (5) `DISABLE_GCS_SYNC=true` en el log del proceso nuevo.
- **⚠ AVISO PARA SAILOR:** es un fallo de CÓDIGO, no de la BD de Álvaro — cualquier entorno `DB_PROVIDER=local` con duckdb moderno (≥1.2 aprox.) tiene la creación de datasets rota por la vía POST desde siempre que tenga esa versión (en la máquina de Álvaro, desde el 7-sep con duckdb 1.5.5; con la 1.1.3 pineada funcionaba). Síntoma: dataset creado con 0 pares y `precache-status` en `error` con «Binder Error: infinite recursion… view "tickers"»; los pares aparecen después «sola» si corres un backtest encima. Fix: commit `cd180fa` en `alvaro-rama-desarrollo` (2 ficheros, rama local de `_init_connection_views` + `init_db`) — llevar a `staging` por PR cuando Álvaro lo decida.
- **HIPÓTESIS (sin investigar ahora, pedido por Álvaro):** duckdb 1.5.5 se instaló en el venv el **7-sep 15:10** y el PRIMER `.wal` corrupto de `local_data.duckdb` es del **7-sep** (`local_data.duckdb.corrupto_20260907.wal`, 1,9 MB; ese día se reconstruyó la BD). Los hallazgos 03/04 (WAL dañado / WAL stall de 160 B en cada parada abrupta) podrían estar relacionados con el cambio de versión de duckdb (formato de WAL/checkpoint distinto entre 1.1.3 y 1.5.5) y no con los reinicios en sí. Por verificar otro día: reproducir el stall con 1.1.3 vs 1.5.5 en un ciclo arranque/taskkill.
- **Código tocado:** `backend/app/database.py` + `backend/app/init_db.py` (con OK explícito de Álvaro, en su rama, commit `cd180fa`, sin push). Backups previos en `.tmp_bloque1/backup_hallazgo11/`.
- **Estado:** 11 y 12 → RESUELTO (commit `cd180fa`). Hipótesis WAL/duckdb → ABIERTA (punto nuevo de investigación).

### [HALLAZGO · 2026-09-25 · 14] Un backtest sobre un dataset_id INEXISTENTE «succeeds» con 0 días y 0 trades en vez de fallar con 404 — y el guard anti-doble-run devuelve luego «already_running» con el job viejo
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** mejora (robustez de errores; me costó ~20 min de diagnóstico)
- **Dónde:** `backend/app/services/backtest_orchestrator.py` (fase 1 qualifying: dataset sin filtros → devuelve resultado vacío en vez de HTTP 404) + `_resolve_filters` (`data_service.py:318`: la lectura local devuelve nada, el fallback GCS 403-kea en silencio, y sigue con `filters={}`) + `POST /api/backtest` (guard por dataset)
- **Qué observé:** escribí mal un id de dataset (un carácter: `…a54a` por `…a54f`). El backtest arrancó, el qualifying no encontró el dataset (fila inexistente + fallback GCS 403 sin credenciales) y el job terminó `succeeded (0 days)` con agregados vacíos — un resultado que ENCIENDE la luz verde. Los dos POST siguientes con el mismo id recibieron «already_running» + el job_id del primero (el guard quedó «running» tras el éxito vacío), y sondear ese job devolvió el resultado vacío del primero como si fuera el suyo.
- **Cómo reproducir:** `POST /api/backtest` con `dataset_id` inexistente (p. ej. cambia un carácter de uno real) → job `succeeded` con 0 trades. Segundo POST inmediato → `{"job_id": <el mismo>, "status": "already_running"}`.
- **Evidencia:** jobs `5001d4e2` (0 días) del 25-sep ~18:0x, log `backend_prof.log` 64237-64297 (hot sync 403 → qualifying FAILED → «No qualifying data» → `succeeded (0 days)`); mis re-POST con «already_running».
- **Impacto:** un typo en dataset_id (o un dataset borrado) produce un backtest «perfecto pero vacío» — fácil de confundir con «la estrategia no entra» (familia del incidente del 5-sep documentado en `database.py:113-138`). Sugerencia cuando el dueño lo vea: 404 explícito si `dataset_id` no resuelve filtros, y liberar el guard al terminar aunque el resultado sea vacío.
- **Código tocado:** NINGUNO (confirmado)
- **Estado:** ABIERTO

### [HALLAZGO · 2026-09-25 · 15] `date_to` es EXCLUSIVO en la vía de datasets (`<`) e INCLUSIVO en la vía qualifying (`<=`): un dataset cuyo rango acaba en 31-dic pierde ese día en silencio (~0,3 % de los pares)
- **Reporta:** GLM (ZCode) (para Álvaro)
- **Severidad:** inconsistencia (borde; afecta a la comparabilidad motor↔estudios manuales)
- **Dónde:** `backend/app/services/query_service.py:168` — `timestamp >= CAST(? AS TIMESTAMP) AND timestamp < CAST(? AS TIMESTAMP)` al materializar los pares de un dataset (vía `build_screener_query`), frente al qualifying directo (`_build_where_clause` en `data_service.py`), que cierra con `<=` inclusivo.
- **Qué observé:** preparando la paridad del filtro 1.6, un dataset 2025 creado con `date_from=2025-01-01, date_to=2026-01-01` y regla PMH ≥ 50 + víspera roja dio **1502 pares** contra los **1507** del mismo universo extraído del lago en el estudio del Bloque 1. Las 5 diferencias (diff exacto contra `dataset_pairs` con el backend parado) eran TODAS del `2025-12-31` (ANGH, KBSX, NCL, ORIS, TOPP), 0 solo-motor. Verificación de control con un dataset PMH-only de HOY: 2975 pares contra 2983 del estudio, 8 menos, todas del 31-dic.
- **Cómo reproducir:** `POST /api/data/datasets` con `date_from=2025-01-01`, `date_to=2026-01-01`, regla `pmh_gap_pct >= 50` → `pair_count=2975`; el mismo universo contado sobre la BD/lago con `<= 2026-01-01` da 2983. Los 8 pares perdidos son exactamente los del 2025-12-31.
- **Evidencia:** `.tmp_bloque1/paridad.json` (1502 vs 1507) y `.tmp_bloque1/paridad_diff.json` (solo_estudio = 5 × 2025-12-31, solo_motor = []); dataset de control PMH-only del 25-sep (2975 = 2983 − 8).
- **Impacto:** cualquier dataset cuyo `date_to` cae en el último día que interesa (31-dic es el caso típico) pierde ese día sin error ni aviso. Los datasets históricos del Baúl se crearon con `date_to` = día siguiente, así que no están afectados. Cuando el dueño lo vea, la decisión es unária (elegir `<` o `<=` en una de las dos vías) pero cambia `pair_count` de datasets futuros: fijarlo explícito.
- **Código tocado:** NINGUNO (confirmado; los datasets de prueba usados para medir se borraron)
- **Estado:** ABIERTO

### [FEATURE · 2026-09-25 · FILTRO 1.6 «VÍSPERA ROJA»] `Gap -1 · Day Return %` — day_return_pct del día anterior con umbral movible, en las tres vías y en las tres UIs (para Álvaro)
- **Origen:** implementación pedida por Álvaro tras validar el criterio 1.6 en el Bloque 1 (`docs/INFORME_BLOQUE1_VELA_VISPERA_20260925.md`): la víspera roja (neto RTH < umbral) sube el fade premarket del gap (+2-4 pp, 7/8 años en universo + 3/3 en trades 1B).
- **Qué cambió (`f1e401b` en `alvaro-rama-desarrollo`, SIN push):** (1) `backend/app/services/qualifying_windows.py`: `day_return_pct` añadido a `PREV_DAY_LAG_SOURCES` → alias `lag_day_return_pct_1` generado automáticamente en las TRES vías que comparten esa lista (materialización de datasets vía `query.py`, qualifying local `data_service.py`, qualifying GCS/Parquet `gcs_cache.py` — la de producción). Columna verificada ANTES de tocar: existe en la tabla local y en el parquet del lago de 2019 y 2026 (mismo ETL que escribe GCS). (2) `frontend/src/lib/universoFiltros.ts`: parámetro «Day Return % (RTH, cierre vs apertura)» con descripción que aclara que es intra-RTH (NO contra el cierre anterior), mapa `gap_prev_day → lag_day_return_pct_1` y helper `paramsDisponibles(sección)`. (3) `InlineDatasetBuilder.tsx` usa ese helper (catálogo por sección). (4) `InlineStrategyBuilder.tsx`: opción en el desplegable de Gap -1, reset del parámetro al cambiar de día-sección, default `0.0`, etiqueta en el resumen. (5) `genetico/page.tsx`: dropdown por sección.
- **Verificación:** tests del filtro + `qualifying_windows` + mapa de métricas → **22 passed** (nuevo `test_prev_day_return_rule_filters_pairs`: víspera roja < 0 deja solo los ticker-días esperados) · `npx tsc --noEmit` limpio · **paridad con el estudio**: dataset 2025 PMH≥50 + `lag_day_return_pct_1 < 0` → motor **1502** vs estudio **1507**; diferencias 5/5 del 31-dic, causa el hallazgo 15 (date_to exclusivo, preexistente, no del filtro) · **navegador end-to-end** (localhost:3000 → backend 8010): la fila aparece SOLO en el bloque GAP-1 DAY (no en GAP+1/+2), operador `<` + valor 0 + Incluir, guardado con «Guardar y Probar» → el dataset llegó a la API con la regla `{"metric": "lag_day_return_pct_1", "operator": "LESS_THAN", "value": "0"}` y materializó **1847 pares** (en esa prueba el PMH ≥ 50 se añadió también en GAP-1 → `lag_pmh_gap_pct_1`, universo distinto al de paridad, 1847 es coherente con él); dataset de prueba borrado después.
- **Nota operativa:** no existe endpoint DELETE de datasets; la limpieza del de prueba requirió parar el backend (ciclo autorizado taskkill+watchdog) y borrar en `users.duckdb` la fila de `saved_queries` + las de `dataset_pairs` (la definición vive en `saved_queries`, no en `datasets`). Un taskkill deja a veces un WAL que el watchdog no supera solo (patrón hallazgos 03/04): renombrado a `local_data.duckdb.wal.stalled_feat16_ui_1855`, watchdog rearrancado, `DISABLE_GCS_SYNC=true` verificado.
...
- **La consecuencia grave:** el `ping` de Massive lo contesta el mismo bucle que lee. Con el p90 en 30,78 s no llegaba y Massive cortó (`1011`) a las 14:05:32 sin intervención de nadie. Cada corte nuestro reconecta a los 5 s, se solapa con la conexión zombi, la cuenta pasa del tope y **Massive echa al socio**.
- **El arreglo barato (tres líneas):** una evaluación **por segundo y ticker**, no por print. El print se aplica igual. Medido en vivo con 380 prints/s: CPU 97 % → **7,7 %**, mediana de la alerta 15,36 → **2,03 s**, p90 30,78 → 2,26 s, tardíos 22.594 → **0**.
- **Los dos cinturones:** `ping_timeout=60` en el websocket (valía 20 s por defecto, igual que `ping_interval`) y `ESPERA_RECONEXION` 5 → **15 s** (los 5 s del 21-sep asumían que la zombi era del socio; cuando es nuestra, volver al segundo 5 le cuesta la conexión a él). Las velas del hueco las recupera `al_reconectar` por REST.
- **La arquitectura:** `bot_alerts_proceso_base.py` (nuevo: tubería, cola acotada que tira lo viejo, resurrección con estado, `Vigilante`), `bot_alerts_alerta_proceso.py` (motor de alertas + hidratación) y `bot_alerts_prealerta_proceso.py` (prints, vela a medias, el recorte por segundo). El bot queda como **LECTOR**: una sola conexión a Massive y reparto por tuberías locales. **Condición de diseño: el lector no calcula nada** — su coste es constante y no crece con tickers ni estrategias.
- **Por qué el de prealertas lleva su propia copia del motor:** `evaluar_parcial` mide contra el día acumulado, así que necesita las velas de minuto igual que el de alertas. Y por eso resuelve sus prealertas él solo (obtiene los mismos eventos) sin preguntar a nadie — pero **no las publica**: publica solo el bot, para que un aviso no salga dos veces.
- **Medido en producción, 4 horas de RTH, 48 ventanas:** mediana de las medianas **2,28 s**, p90 típico 2,44 s, peor p90 3,66 s, 45/48 por debajo de 2,5 s; rato sin leer 0,48 s de mediana; 1.196.326 prints → 35.855 evaluaciones (1 de cada 33); cero cortes y cero resurrecciones. Las 8 alertas en el **segundo 1-2**. CPU **8,3 % en 4 núcleos**, RAM 682 MB (antes 330).
- **Lo que NO gana:** velocidad. 2,28 s frente a los 2,03-2,15 s del parche solo es la misma cosa; ~2 s es el suelo de Massive. Gana **margen** (nada caro en el bucle que lee; `[TIEMPOS]` desaparece del log) y es la base del bot automático.
- **Las trampas, y dos en las que caí:** (1) mandar **un mensaje por print** desbordaba la cola — de 800 prints llegaban 21 y la vela a medias salía falsa; va en lote cada 50 ms, con tope de 30.000 por si el hilo de vaciado muriera. (2) la recuperación tras un corte recorría `runner.tickers`, vacío en el diseño nuevo: no habría recuperado ninguna vela. (3) el hijo de prealertas duplicaba líneas en el log (no en Telegram ni en el cuadro de mandos): su logger va silenciado.
- **Pruebas:** `test_bot_alerts_procesos.py` (8: velas y respuesta por cada una, hidratar no bloquea al bot, el recorte por segundo con 800 prints, el de prealertas no publica cierres, no bloquear con el hijo muerto ×2, el vigilante resucita con estado, el resumen). 253/253 de `bot_alerts` en verde.
- **Retención de grabaciones:** 30 → **60 días** (53 MB en 16 sesiones, van en gzip).

### [INTEGRACIÓN · 2026-09-23 · SAILOR] Traído lo de Álvaro por cherry-pick: TP por lote, Fase 2 de picos, etiquetas y dos fixes
- **Vía:** cherry-pick selectivo desde `alvaro-rama-desarrollo` siguiendo su `PRD_PARA_SAILOR_TRAER_ALVARO_20260923.md`, más un merge de `origin/staging` (sus docs, su MEMORIA_MADRE y la compartida B200 `aa06`). Nunca un merge de su rama entera: mantiene 17 ficheros del bot ausentes.
- **Lo traído:** `328bc0d` (Fase 2 de picos y valles al carril nativo, apagado por defecto), `149cacb` (dropdown `ap_session` mostraba RTH con el motor en PM), `38fa515` (modal del calendario: R total y media etiquetada), `eaa7ce1` (**TP por lote**, `lot_tp`, + PRD) y `9016fb1` (**etiquetas** de estrategias).
- **El conflicto real y su resolución** (decisión de Jaume: «el nuestro va por delante»): su `portfolio/page.tsx` llevaba aún la pestaña `PortfolioTab`, borrada aquí el 21-sep al dejar Portfolio en dos pestañas. Se conservó NUESTRA estructura y solo se añadió su `onTags={etiquetar}`. Los demás conflictos eran de `MEMORIA_MADRE.md`, espurios: sus entradas ya habían llegado con el merge de staging.
- **Lo que cierra `eaa7ce1`:** sin él, una estrategia con `lot_tp` guardado **lo ignora en silencio** (el bloque se cae al hidratar la definición, sin error). Un «bug que no da error» de manual.
- **Migración local hecha:** `ALTER TABLE strategies ADD COLUMN IF NOT EXISTS tags VARCHAR` en `backend/users.duckdb` (copia en `users.duckdb.bak-2026-09-23`; 19 estrategias intactas). Sin ella, `test_strategy_api` da `Binder Error: Referenced column "tags" not found`.
- **Verificación:** **1.691 pasan, 32 se saltan, 2 fallan** y los dos son PREVIOS: `test_filtros_metric_map` (METRIC_MAP apunta a 5 columnas que faltan en `daily_metrics`, vienen de `13e901af`, los filtros de volumen contra el universo) y `test_prefetch_parity` (busca `indicators._ticker_daily_ohlc_cache`, inexistente también antes del merge). Comprobado que ningún cherry-pick toca `METRIC_MAP`/`daily_metrics` y que el cambio de `indicators.py` es solo comentarios. `npx tsc --noEmit` limpio.
- **Zona sensible:** 398 tests verdes de bot de alertas, pirámides, TP por lote, etiquetas y picos. Importaba porque `eaa7ce1` toca `strategy_engine.py` y `portfolio_sim.py`, dos de los tres ficheros compartidos con el bot en vivo.
- **Estado:** sailor y staging **a la par**. Pendiente de Álvaro, no traído: el feat de Scalping UI (`62fc54e`); en staging solo su PRD.

### [FIX · 2026-09-24 · BOT DE ALERTAS] El SL y el TP POR LOTE de la piramidación se avisaban como «AÑADIR» — arreglado SOLO el reconocimiento
- **Para el chat del bot:** la ARQUITECTURA NO SE HA TOCADO (lector/radar/alerta/prealerta/vigilante, tuberías, publicación, formato de los avisos: todo igual). Solo se enseñó al motor de avisos a reconocer dos tipos de ejecución que el simulador ya emitía. Si algo raro sale en un aviso de pirámide, mirar aquí primero.
- **El fallo:** el simulador apunta el cierre de un lote en `pyr_executions` como `kind: "lot_stop"` (SL por lote, 17-sep) o `"lot_tp"` (rungs del TP por lote, 22-sep) Y además emite la leg de cierre («Pyramid Lot Stop» / «Lot TP (n/N)»). El bot solo conocía `add`/`reduce`: todo lo que no fuera `reduce` lo sumaba como añadido. Reproducido (entrada 284 + add 100, salta el SL del lote): **«➕ AÑADIR 100 (posición total 484)»**, luego «CIERRE PARCIAL 124» y el cierre final «360» cuando se tenían 284. Sin error y sin log.
- **El arreglo** (`bot_alerts_engine.py`): `_KINDS_LOTE`, `_es_leg_de_lote`, `_clave_ex`. `_total_avisado` resta los cierres de lote; la leg del lote no se avisa además como salida (ya va como pirámide, igual que antes iban los add); cantidad topada a lo avisado y, si el lote vacía la posición en el simulador, se cierra todo lo avisado y la señal se rearma; la prealerta no avisa cierres de lote (mismo criterio que las salidas). Etiquetas: «➖ STOP DE LOTE» / «➖ TP DE LOTE» en `bot_alerts_telegram.py` y «Stop de lote» / «TP de lote» en la tabla de `CuadroMandos.tsx`.
- **Dos trampas cubiertas:** el SL del lote puede saltar en la MISMA vela del fill del add (mismo nivel y vela: la clave vieja `(nivel, vela)` se comía uno de los dos) y dos rungs del TP en la misma vela.
- **add y reduce: salida idéntica byte a byte** a la de antes (comprobado con el mismo guion). Decisión de Jaume: «añadir es añadir; solo TP y SL del lote».
- **Anotado y NO tocado (previo, decisión de Jaume):** un `reduce` se avisa dos veces — «➖ REDUCIR 100» y además «CIERRE PARCIAL · Pyramid Reduce 73» — y el cierre final descuadra. Es el mismo patrón que tenían los lotes; si algún día se arregla, la receta es la misma (`_es_leg_de_lote` ampliado a «Pyramid Reduce»).
- **Pruebas:** `tests/test_bot_alerts_lote.py` (5, vela a vela con el `simulate` REAL: SL de lote, SL en la vela del fill, dos rungs en la misma vela, avisado ≠ simulador, prealerta). Suite completa **1.612 pasan, 0 fallan**; `npx tsc --noEmit` limpio.
- **Cuándo llega al bot:** en el próximo arranque (`bot.py` importa de `D:/Backtester/backend` al arrancar). No se reinició nada; el bot estaba parado.

### [FIX · 2026-09-24 · BOT DE ALERTAS · 2] REDUCIR también se avisaba dos veces — arreglado con la misma receta
- **Pedido por Jaume** tras la entrada anterior («de cara a reducir o agregar mejor que funcione bien; y de cara a un bot automático puede dar problemas»). Arquitectura del bot, otra vez, INTACTA; añadir sin cambios.
- **El fallo:** el simulador anota cada reducción como pirámide (`kind: "reduce"`) y además como leg de cierre («Pyramid Reduce»). El bot avisaba las dos: con entrada 200 y reducir 50 %, «➖ REDUCIR 100 (posición total 100)» + «CIERRE PARCIAL · Pyramid Reduce 50 (quedan 50)» + cierre final «50». Quien hacía caso solo al primero se quedaba con 50 acciones abiertas tras un «CIERRE POS.»; quien hacía caso a los dos llevaba media posición desde la reducción.
- **El arreglo** (`bot_alerts_engine.py`): `_es_leg_de_lote` → `_es_leg_de_piramide` (también «Pyramid Reduce»: su leg ya no se avisa como salida). Nuevo `_libro_avisado`: el recorrido de la posición AVISADA ejecución a ejecución — añadido = lo del simulador redondeado (como siempre); **reducción = la misma fracción que en el simulador pero sobre lo avisado** (como los tramos parciales), antes era el número de acciones del simulador; cierre de lote = el lote tal cual; si el simulador vacía la posición, se cierra todo lo avisado y la señal se rearma (antes, un reduce que vaciaba la posición la dejaba sin rearmar porque el rearme solo lo ponía el aviso de salida). `_total_avisado` lee de ahí.
- **Pruebas:** 4 casos más en `tests/test_bot_alerts_lote.py` (reducir sola, añadir y reducir, avisado ≠ simulador, reducir el 100 % + rearme). Suite **1.616 pasan, 0 fallan**.
- **Impacto hoy:** ninguna de las dos estrategias vigiladas (PM 1A TTP, PM 1A Álvaro Real) reduce ni lleva lotes: ningún aviso real salió mal.

### [INTEGRACIÓN · 2026-09-25 · MERGE STAGING a652c7d → alvaro-rama-desarrollo] Bot de alertas a 4 procesos + fixes de avisos de pirámide + libro del bot; zona bot TAL CUAL staging
- **Qué entra:** 41 commits de `origin/staging` (68d4c2a → a652c7d), casi todo zona bot de alertas y docs: (1) arquitectura de 4 procesos (lector único de Massive + prealerta + alerta + vigilante, `bot_alerts_proceso_base/alerta_proceso/prealerta_proceso.py`) — el cuello era evaluar prealertas con cada print (97 % CPU) y ahora una evaluación por segundo y ticker; (2) el bot ya reconoce `lot_stop`/`lot_tp` (antes los avisaba como «AÑADIR») y REDUCIR ya no se avisa dos veces; (3) la recuperación tras un corte no repite velas en las prealertas; (4) libro del bot de ejecución (`BOT_EJECUCION_*`: rutas del bróker DAS/Sage, SPAC fuera siempre / IPO <30 d fuera en RTH, márgenes de stops, horarios); (5) la INTEGRACIÓN de Sailor (mis feats ya viven en staging: TP por lote `eaa7ce1`, Fase 2 picos, tags, dos fixes).
- **Conflictos: 1** (`docs/MEMORIA_MADRE.md`) — resuelto conservando TODAS las entradas de los dos lados (mi bloque 23-25-sep + su bloque ARQUITECTURA/INTEGRACIÓN/FIX del bot). Cero conflictos de código: los cherry-picks de staging ya casaban mi lado.
- **Restaurado en el merge:** `backend/tests/test_evf_paquetes_de_100.py` — `015cb93` (commit docs de HOY 19:13 de la sesión paralela) lo borró por accidente (se ve un `git add` con el fichero ya ausente localmente; quedó un `.pyc` huérfano en `__pycache__`). Traído de `origin/staging` (idéntico al merge-base). **Aviso para Álvaro: ojo con `git add -A` en esa otra sesión.**
- **Zona bot verificada TAL CUAL staging:** diff VACÍO contra `origin/staging` en los 5 services + 5 tests + `bot_alerts.py`/`bot-alerts/`/`bot-alertas/`/`CuadroMandos.tsx`/`BOT_ALERTAS_MODOS_DE_FALLO.md`. Porta-verja de borrados: vacío tras restaurar el evf. El bot NO se arrancó (regla).
- **Mis fixes siguen dentro, sin pisar:** `cd180fa` (vistas `massive.*` 3-part: `current_database()` presente en `database.py` e `init_db.py`) y `f1e401b` (filtro 1.6: `day_return_pct` en `qualifying_windows.py` + `universoFiltros.ts`).
- **Verificación:** suite completa **1.638 passed, 118 skipped, 3 xfailed, 0 fallan** (57 s; antes del merge 1.585 en el merge anterior — crecen los ~30 tests nuevos del bot) · `npx tsc --noEmit` limpio · backend rearrancado con el ciclo autorizado (taskkill+watchdog): health OK, `DISABLE_GCS_SYNC=true` en el log, arranque limpio. El taskkill de las 19:25 volvió a dejar un WAL de 160 B que el watchdog no supera solo → renombrado a `.wal.stalled_merge_1929` (recurrencia SISTEMÁTICA del patrón de los hallazgos 03/04; hoy 3 veces).
- **Prueba de regresión del fix y del filtro juntos:** `POST /api/queries/` con `TEST_MERGE_1_6_borrar` (jun-2025, `pmh_gap_pct ≥ 50` + `lag_day_return_pct_1 < 0`) → **170 pares materializados por la vía POST** (la que arregló `cd180fa`); borrado con `DELETE /api/queries/{id}` → 200 y lista limpia.
- **CORRECCIÓN de mi FEATURE del filtro 1.6:** SÍ existe endpoint DELETE de datasets — está en `/api/queries/{id}` (no en `/api/data/datasets/{id}`, que dio 405) y hace borrado en cascada (`saved_queries`+`datasets`+`dataset_pairs`+`precache_state`). La cirugía de BD de esta tarde no era necesaria.
- **Qué trae Jaume que me afecta (resumen llano):** (1) los avisos del bot de pirámides ahora cuadran — SL y TP de lote se etiquetan como tal y REDUCIR no se avisa doble: si opero con avisos, releer el formato nuevo; (2) el bot ya no echa la conexión de Massive del socio cuando se cae (reconexión a 15 s y velas del hueco por REST): menos sustos a las 09:00; (3) mis estrategias con lotes (TP por lote) ya se avisan bien en el bot — hasta hoy se avisaban como AÑADIR; (4) el libro del bot fija reglas de universo que me tocan: SPAC fuera SIEMPRE y IPO <30 días fuera en estrategias RTH (medido: SPAC = 26 % de los T12 y perdiendo); (5) staging ya contiene casi todo mi código (Sailor lo cherry-pickeó) — lo mío pendiente de integrar: scalping UI (`62fc54e`), fix datasets (`cd180fa`) y filtro 1.6 (`f1e401b`).
- **Estado:** HECHO — merge commiteado en `alvaro-rama-desarrollo`, SIN push (pendiente OK de Álvaro). `staging` y `main`: intactos.

### [CORRECCIÓN · 2026-09-25 · INTEGRACIÓN de hoy] `015cb93` NO borró `test_evf_paquetes_de_100.py` — seculos drops en CADA merge mío de staging desde el 9-sep; mi script de detección tenía un bug y culpó al primer commit examinado
- **Reporta:** GLM (ZCode) (para Álvaro), tras su aviso.
- **Qué estaba mal en mi entrada INTEGRACIÓN de hoy:** afirmé que `015cb93` (commit docs de la sesión paralela, 19:13) «borró por accidente» el test y especulé con un `git add -A`. **FALSO**: `git show --stat 015cb93` → un solo padre (`72f2e63`) y un solo fichero (`docs/MEMORIA_MADRE.md`, +9 líneas). Un commit así no puede borrar nada. El error vino de MI script de rastreo: inicialicé la variable «presente» a `True` en vez de comprobar el primer commit, así que el walk culpó SIEMPRE al commit más nuevo examinado (que era `015cb93`). El mensaje del merge `c183921` arrastra la misma afirmación errónea.
- **Causa real, con evidencia (`git cat-file -e <rev>:<fichero>` commit a commit):**
  1. El test nació en staging el **8-sep 14:52** (`5cd4b42`, feat cangrejo+evf) y fue **BORRADO 19 minutos después** (`1a3cf21`, 15:11, fix del bot de Jaume).
  2. El merge **`baebe92`** (sailor→staging, el mismo minuto) lo **RESUCITÓ**: la resolución conservó la copia del lado sailor aunque staging lo acababa de borrar. Por eso `git log --all -- <fichero>` solo muestra 2 commits (los merges no salen sin `-m`) y el fichero existe en el tip de staging.
  3. En MI rama, el fichero falta en el RESULTADO de **todos** los merges de staging verificados: `e25df90` (9-sep), `4c939f3` (18-sep) y `f5b1be7` (23-sep) — en los tres, el lado staging del merge (p. ej. `e25df90^2`, `68d4c2a`) SÍ lo tenía. Es decir: **no hubo un único evento de pérdida** (la hipótesis «se perdió en f5b1be7» tampoco es exacta: en `f5b1be7^` = `9016fb1`, mi lado ANTES de ese merge, ya faltaba): cada resolución de merge repetida lo fue dejando fuera desde el 9-sep.
  4. Confirmado lo que dijo Álvaro: ya faltaba en `e627e2e` y en toda la cadena hasta `e25df90` inclusive.
- **Estado actual:** el merge `c183921` de hoy lo trajo de vuelta (zona bot = staging literal, y el test venía con ella); `5236cbe` (REGLA EN PIEDRA) y el árbol actual lo conservan. Nada que reparar.
- **Lecciones (ambas mías):** (1) los commits de docs de una sola línea no son sospechosos de borrar ficheros — verificar con `--stat` ANTES de acusar; (2) un script de rastreo que «encuentra» como culpable al primer commit que mira casi siempre tiene bug de inicialización. La aleación correcta para rastrear desapariciones: `git cat-file -e <rev>:<fichero>` commit a commit SIN atajos, y `git log --full-history -m -- <fichero>` para ver qué merges tocaron el fichero.
- **Código tocado:** NINGUNO (esta corrección y el push autorizado de la rama; `staging` intacto).
- **Estado:** CORREGIDO (la entrada INTEGRACIÓN anterior queda como estaba, con su error señalado aquí).
