...
- **PENDIENTE para verlo en la página: reiniciar el backend** (`/api/genetico/catalogo` importa el módulo una vez por proceso y `_genes_extra` es del router). **Reiniciado a las 12:45 por orden de Jaume** (bot en vivo; es proceso aparte y siguió): catálogo nuevo servido (51 indicadores, `params_fijables`). **Subido** en `4e6a629` a `sailor-rama-desarrollo`, junto con los 7 commits del otro chat que estaban sin pushear (`3128cbd`…`d9ee937`).
