...
- **Subido el 20-sep a las 22:44** (Jaume: «lo que falte subir para staging y sailor hazlo»): `sailor-rama-desarrollo` de `f561623` a `413c67a` (35 commits: todo el crudo v4 del día, sus MEMORIAs, y los docs del bot de ejecución de la otra sesión) y `staging` en fast-forward de `fcf6c21` a `413c67a` (staging era ancestro: comprobado con `merge-base --is-ancestor` antes del push, 70 commits). Nada pendiente de subir al cerrar el día. Sobrantes por limpiar cuando toque: `limitesHistoricos` (ya no lo usa la UI).
- **23:15, gráficos del paso 3 nivelados** (`30295b3`, subido a sailor y staging): modo «plano» en `SpaghettiChart`/`DistributionChart` (sin caja redondeada, marco cuadrado, márgenes de LinesChart) y alturas que cuadran; robustez y el portfolio viejo siguen igual. **Noche del 20 al 21 (Jaume dormido, «al mando»)**: vigilancia de la corrida del genético `20260920_231104_09bc` «Genético 2» (1 worker, 42 s/eval, ETA 23 h, SIN hora de parada) cada 15 min con `scratchpad/vigilar_genetico.py` (relanza por `POST /reanudar` si se cae); sin parada automática (Jaume, 23:30: la para él a mano; min_trades 1000 a propósito). La otra sesión: no hace falta tocar el backend por la noche.

## 2026-09-21 (Sailor, Genético, tarde) — Picos/valles enumerados, «Ultimo pivote», Prev. Bar y Previous max/min: con casilla en explorar y con genes en mejorar

- **Qué pidió Jaume** («no tiene en cuenta indicadores como el pico/valle (nºN) o el Ultimo pivote, previous max y min, Prev. Bar close/open/high/low, el de recorrido, volumen del pico o edad del pico… que funcionen bien»; va a relanzar el genético parado). Diagnóstico: en **explorar**, los nueve niveles base (Prev. Bar ×4, Previous max/min, VWAP, PM High/Low) SÍ entraban como destino de Bar Close / High Bar / Low Bar pero **sin casilla** (ni se veían ni se podían quitar) y la familia de picos (PRD 18-sep) **no estaba en el catálogo**; en **mejorar**, `extract_parameters` no sacaba `pivot_window` ni `pivot_rank` (una estrategia con «Pico nº2» no tenía NADA que mover), los objetivos **negativos** («Recorrido (%) < -2», Reg. Slope, ATR Extension) salían con rango **0…8** (max(0, v·0,25) / max(3v, v+10) pensados para positivos: el 3D, el WFO y el genético barrían el signo contrario sin error) y un nivel sin parámetros («Bar Close < Prev. Bar Low») no tenía ningún gen.
- **Explorar (`genetico/catalogo.py`, `cromosoma.py`, `motor.py`).** (1) Los nueve niveles base son entradas del catálogo en «Precio y niveles», `solo_destino`, **marcadas de serie**; la página manda `niveles_explicitos: true` y entonces solo entran los marcados. `objetivos_permitidos(ind, marcados, explicitos=None)`: sin la clave (corridas antiguas, consola, tests) se deduce —si `marcados` no trae ningún nivel base se sortean los nueve **en el mismo orden de siempre** (test `test_sin_marcar_nada_los_destinos_son_los_de_siempre` sigue igual). SMA/EMA/bandas/Donchian/Darvas/Overhead entran siempre (garantiza que nunca queda vacío). (2) **«Pico»** con dos papeles y un interruptor: a la izquierda contra otro Pico (`pivot_rank` 1-3; el destino hereda ventana y dirección y cambia SOLO el nº de giro —`objetivo_aleatorio(..., params_izq)` y `cromosoma.coherencia(c)` tras mutar el origen— porque techo contra suelo es siempre verdad y el mismo giro contra sí mismo siempre falso), y como destino opcional de Bar Close/High/Low con giros **2-4** (el 1 ES «Ultimo pivote»: ofrecerlo sería el mismo gen con dos huellas). (3) **«Edad del pico»** (5…120 min) y **«Volumen del pico»** (100 k…2,5 M acciones, como viaja en la definición) como medidas en Alternativos. (4) De paso, al darles casilla el test de ramas cazó que **«Previous max/min» tienen `ap_session`** y el genético nunca lo mandaba → el motor contaba desde las 04:00 (ap.PM) mientras el constructor pone ap.RTH: la misma receta guardada desde el genético y hecha a mano no era la misma estrategia. Ahora se sortea `["ap.PM", "ap.RTH"]`. (5) `catalogo.etiqueta()` pinta «Pico nº2(3, up)». `TODOS_LOS_NIVELES` sin repetir Previous max/min.
- **Mejorar (`optimization_service.py`, `routers/genetico.py`).** `_extract_indicator_params` saca `pivot_window` (1-8) y `pivot_rank` (1-6, etiquetas «velas de confirmación» / «nº de giro»); los dos en `_INT_PARAM_KEYS`. **Rango de objetivos negativos reflejado** (−2 → de −12 a −0,5; `_auto_step` con `abs`). Lo gana el 3D también. Gen **categórico «nivel contra el que compara X (hoy Prev. Bar Low)»** en `_genes_extra`: solo cuando el destino ya es uno de los nueve niveles base, opciones = los nueve, `path = …target.name` (la misma numeración por posición que `extract_parameters`; las guardas se meten después, así que vale). Cambia la estrategia, como los parciales: empieza desmarcado.
- **`ap_session` sigue a la sesión de la corrida, y se puede fijar** (segunda petición de Jaume: «previous max y min tienen que tener en cuenta la sesión en la que fijo que se corran las pruebas, y mejor si puedo seleccionar el periodo… igual que el fade»). Comprobado en `backtest_service`: los indicadores se calculan sobre el día entero y la sesión recorta DESPUÉS, así que en RTH un Previous max desde las 04:00 incluye el PM. `config["params_fijos"] = {indicador: {param: valor}}` con `"auto"` (según la sesión: rth → ap.RTH, pre → ap.PM, post → ap.AM, custom → por la hora de inicio, varias → la más temprana; `catalogo.sesion_de_referencia`), `"*"` (sortear la rejilla) o un valor fijo. `catalogo.rejilla_params(nombre, config, base)` reduce la rejilla a un valor y se usa al nacer, al sortear el destino y al mutar (`_mutar_condicion` solo muta params con >1 valor: un fijo no es gen). Sin la clave (corridas antiguas) se sortea como siempre. Hoy fijable: `ap_session` de Previous max / Previous min / % Fade (`PARAMS_FIJABLES`, `OPCIONES_FIJABLES`; el endpoint del catálogo lo expone como `params_fijables`). En la página, al marcar uno de los tres aparece «cuenta desde» con el desplegable; **la página manda siempre los fijables marcados con su defecto («según la sesión»)**, así que sin tocar nada Previous max ya va alineado. La rejilla del % Fade pierde el `None` (era lo mismo que ap.PM). Tests: 5 nuevos en `test_genetico_catalogo.py`.
- **Página** (`genetico/page.tsx`, `api_genetico.ts`): `niveles_explicitos: true` y `params_fijos` en el config; nota de los «↳» también en «Precio y niveles»; el aviso de niveles sin Bar Close se compacta si son más de 3; la ayuda del stop lee los niveles del catálogo (decía HOD/LOD, que salieron el 20-sep). `tsc --noEmit` limpio.
- **Verificado:** 253 tests (`test_genetico_*`, `test_offset_cero_es_afinable`, `test_optimize_*`) + nuevo `tests/test_genetico_picos_y_niveles_mejorar.py` (7). Extremo a extremo con `run_backtest` real sobre 3 días sintéticos: todas las recetas nuevas generan operaciones (Pico nº2 > Pico nº1: 58; Bar Close > Pico nº3: 19; Edad del pico nº2 > 15: 2; Volumen del pico nº1 < 250 k: 92; Prev. Bar Close: 133; Previous max(ap.RTH): 3; Ultimo pivote: 94).
- **PENDIENTE para verlo en la página: reiniciar el backend** (`/api/genetico/catalogo` importa el módulo una vez por proceso y `_genes_extra` es del router). **No lo he reiniciado**: el bot estaba en vivo (`bot.py --vivo` desde las 10:05) y no se toca el backend con el bot encendido; lo hace Jaume cuando pare el bot. El proceso del genético sí importa el catálogo nuevo por su cuenta. **Nada commiteado** (8 ficheros modificados + 1 test nuevo, en `sailor-rama-desarrollo`).

## 2026-09-21 (Sailor, Genético) — «Genético 2» daba WR 0 % en todo: el slippage del panel iba ×100

- **Síntoma** (Jaume, 07:15: «no está encontrando nada, ¿hay algún error?»): tras la noche entera (gen 6, 490 evaluadas, sin una sola caída), 474 individuos a fitness 0 y 4 negativos; **ninguno con un trade ganador** (WR 0 %, PF 0, retorno −100 %), incluso con 10.000–40.000 trades. La del 12-sep tenía WR mediana 44,8 %.
- **Causa** (`3128cbd`): la página del genético pide «Slippage %» y manda 0,3 tal cual; `genetico/evaluador.parametros_backtest` se lo pasaba a `run_backtest` sin dividir por 100, y el motor lo lee como FRACCIÓN por lado: **un 30 % de slippage por lado**. El BacktestPanel sí divide (slippage y comisiones PERCENT). La del 12-sep iba con slippage 0 y no se vio. Arreglado en el evaluador (un solo punto de conversión, las dos ramas; FLAT no se toca), test `test_slippage_y_comisiones_del_panel_van_en_por_ciento_y_el_motor_en_fraccion`. Caso 20 de «bugs que no dan error».
- **Relanzada** con el mismo config como corrida NUEVA `20260921_072024_6e11` («Genético 2 (relanzada 21-sep, slippage arreglado)»; reanudar la vieja habría reutilizado su caché envenenada): con Chrome cerrado arranca con 2 workers, 23 s/eval, ETA ~12 h, y el cuarto individuo ya da fitness 0,41. La vieja quedó «parada» (parar.txt) en gen 6. Vigilancia cada 15 min sigue sobre la nueva. De paso: Cangrejo activado en modo «recorrido» sin el % (`cangrejo_max_sl_dist_pct` ausente) no hace nada; `min_trades` 1000 es decisión suya.
- **09:26: Jaume paró la corrida a mano** («esta noche seguimos»): gen 2, 216 evaluadas, mejor 2,65 (`PM High Gap > 100 AND Vol. de la franja(1.0) > 80 AND Reg. R2(10) < 0.85 · Stop Previous Max (vela de la señal) +15 % · TP 10:00`). Se reanuda desde la página cuando quiera (esta sí, su caché es buena). Vigilancia parada.
- **Portfolio limpio** (`d47355a`, Jaume: «solo la subpágina de crudo y la de Baúl con todas sus funcionalidades, y Baúl pasa a llamarse Análisis de portfolio»): la página tiene dos pestañas, **Baúl** (tal cual: baúl genérico, cuadros Portfolio/Incubadora, curvas, detalle, renombrar, borrar, orden ▲▼) y **Análisis de portfolio** (el antiguo «En crudo», los cinco pasos, montado directamente en la página; corregido tras un primer intento en que renombré el Baúl por error). Borrados: `PortfolioTab` (Imagen general, Modelos de escalado y pesos, Comparativa), `MonitorTab` + `ControlTiempoReal` (control en tiempo real), `ScalingSection`, y los gráficos `PortfolioCurves`, `CompareCurves`, `WeightsArea`, `OverlayRealSim`. Se quedan `StatSheet` (InlineStats lo usa robustez), `PnlCalendar`, `CorrelationMatrix`, `puntero`, `StrategyShelf`, `StrategyDetail`. Comprobado que el bot NO dependía de la Imagen general (elige sus estrategias en su propia tabla de `bot_alerts_service`); el comentario del crudo que decía lo contrario estaba desfasado. El cuadro «Portfolio» del baúl sigue valiendo: es lo que marca el botón «las del cuadro Portfolio» del paso 1. **Pendiente**: quitar del backend los endpoints huérfanos de `portfolio_lab` (run/modelos/comparar/monitor) y las funciones de `api_portfolio_lab.ts` que ya nadie llama: exige reiniciar el backend, y hoy es día de bot (10:10). Hecho con el genético parado y `next dev` vivo (2,4 GB libres); tsc limpio; las dos pestañas comprobadas en el navegador.
...
- **«¿No hay modelo que mejore variando los pesos según vayan? ¿Y cuándo ir menos expuesto?»** (Jaume, 13:30). Medido antes de construir (motor entero donde se podía): (1) **rotar los pesos por ranking** (cada mes, ventana de 6 meses, la mejor 4 / la del medio 2 / la peor 1) ×841 frente a ×651 fijo con la misma caída, ganando en las dos mitades; funciona porque la calidad de una estrategia persiste (2A «la peor» el 75 % de los días, 7 cambios de ranking en 33 meses); Kelly por ventana, «apagar la que va en negativo» y la contraria, peor. (2) **Freno por caída de la cuenta** (−10 % → ×0,5 hasta −5 %): ×466 con DD −13,6 % frente a ×256 de un constante con la misma DD: solo frena dentro de las rachas malas; no evita un precipicio de un día. (3) **Lo que no**: ir menos expuesto los días con muchas señales (los días con más exposición son los MEJORES: > p95 +4,8 % de media y 9 % de perder; los peores días fueron tranquilos: el −9,3 % con 7 posiciones), el tope de exposición diaria (solo quita dinero), y el límite de pérdida diaria de la cuenta (bloquea los trades que recuperan la tarde: el peor día pasa de −9,6 a −12,9 %).
- **Escalado automático, HECHO** (`cd7a8ab`; Jaume: «ponerlo en la zona de escalado quitando lo de Kelly, que ajuste por semana/mes/N días, ver cómo habría quedado, los pesos del próximo periodo, el freno como opción, mínimos por estrategia, ? explicativos, y con el CSV real cuánto exponer y cuánto a cada una»). `services/escalado_auto.py`: `Rotacion` (ordena por lo rendido POR UNIDAD de tamaño en la ventana —la sombra: R neta por acción en su unidad × 0,01, así una estrategia apagada puede volver a subir—, patrón por puesto, suelo que conserva la suma, primer periodo con los % del paso 1, `hoy`), `Freno` (decide con el cierre anterior) y `freno_sobre_curva` (la cuenta real); en el motor, `cfg["rotation"]` y `cfg["brake"]` dentro del bucle diario. UI: paso 4 «Escalado» = A nivel · **B automático** (rotación + freno, «Siguiente periodo — lo que toca poner», fijo vs automático, periodos) · C frontera de repartos fijos · D setups; paso 5 reparte con los pesos de la rotación (o los del paso 1), aplica el freno a la curva real y termina en «Siguiente periodo — lo que toca poner en tu cuenta». En sus PM: rotación mensual 126 → 5,81 M (DD −15,9 %) vs 4,95 M fijo; semanal 6,67 M; freno 10/×0,5/5 → 2,90 M con DD −13,6 % (42 días frenado, 4 episodios); hoy: 1A 4 / 2A 1 / 3A 2 (ranks 1, 3, 2); su cuenta real no está frenada (en máximos). Tests `test_portfolio_crudo_escalado_auto.py` (6); suite 1.459 verde; `tsc` limpio. Backend reiniciado a las 14:02 tras esperar a que Jaume terminara un backtest (cpu a 0 y sin conexiones 30 s seguidos: esa es la comprobación antes de reiniciar).
- **Caminos según los locates, Markowitz fuera y la métrica de la rotación** (`c8a02d3`; Jaume, 15:00). (1) «Markowitz hemos dicho que no sirve»: fuera de la frontera (candidatos, columnas, función y test); queda una línea en la ayuda diciendo por qué se quitó. (2) «¿Pesos según la que mejor gana en el periodo o cómo?»: la rotación ordena por **lo que ha hecho cada estrategia en la ventana**, no por su mejora: «lo ganado por 1 %» (lo medido), «EV por trade» (nuevo: lo ganado por unidad ÷ sus trades: calidad media sin premiar a la que opera más) o Sharpe; la ayuda lo explica. (3) **Caminos de la simulación según los locates** (`POST /raw/caminos`, bloque en el paso 3): la configuración del paso 1 corrida ENTERA N veces con semillas distintas del sorteo (con puerta cambia también qué cortos entran), para el rango del paso 1 y los que se añadan (0,3–15 $ el del bróker real); bandas p05–p95, final, caída, coste y trades por rango. En sus PM, 30 semillas: rango 1–10 $ → final p05 2,25 M · p50 3,79 M · p95 5,20 M (**su semilla, 4,95 M, está cerca del p95: algo de suerte con los locates**), DD p50 −16,2 %; 0,3–15 $ → p50 5,92 M; 90 s las dos. Y en el paso 5, **«Tu cuenta real sobre los caminos»**: la curva del CSV (capital inicial + PnL diario) encima de la banda en los días comunes, re-basadas a 0 % el primer día real (el «cruce real-simulado» que esperaba desde el 16-sep; hoy el tramo común es corto porque sus corridas acaban el 4-sep: hay que correr las estrategias hasta hoy). Backend reiniciado a las 15:51 (ocioso 30 s).
- **Simplificación final del paso 4** (Jaume, 16:30: «sigo sin entender la partición; ¿por qué añades Kelly y demás? metemos solo eso; explícame el freno; y con el CSV cuánto ir de cargado con o sin freno y cuánto a cada estrategia; y la curva con solo rotación, solo freno y las dos»). Fuera el tamaño por setup (motor, `setup_sizing.py`, test), `/raw/reparto` (Kelly conjunta, frontera) y sus bloques. **Paso 4 «Escalado» = A · Nivel por la caída (para elegir el tope) + B · Escalado automático** (rotación: cada semana/mes/N, ventana X, «por qué se ordena» = lo aportado por cada 1 % en la ventana | EV por trade | Sharpe, patrón por puesto cuya suma es el tope global, suelo; freno con/sin y regla). Calcular corre las combinaciones: solo rotación, solo freno con pesos fijos, y las dos, frente al fijo: cuatro curvas + por año + «Siguiente periodo — lo que toca poner». Ayudas reescritas «para retrasados»: la partición con ejemplo (1.º del ranking → primer número del patrón…), el freno con ejemplo (máximo 100.000 → cae a 89.000 = −11 % → desde el día siguiente todo a la mitad, 4/2/1 → 2/1/0,5 → hasta volver a 95.000 = −5 % → completo; una caída que iba al −16 % se queda en ≈ −13 %; no evita un precipicio de un día). **Paso 5**: la tarjeta final da la exposición total y el reparto **sin freno y con freno** (el freno aplicado a la curva real del CSV). Lo medido con sus PM (motor entero): fijo 4,95 M / −16,2 %; solo rotación mensual 5,81 M / −15,9 % (semanal 6,67 M); solo freno 2,90 M / −13,6 %; las dos 3,95 M / −13,3 %. Backend reiniciado 16:56 (ocioso).
- **«¿Los pesos los mides en relativo? ¿Y fixed ratio?»** (Jaume, 17:10; `commit siguiente a 30f8f9f`). Sí: el ranking va por lo aportado **por cada 1 % de tamaño**, nunca en dinero; lo que NO contaba era el tiempo en mercado. En esta simulación tener la posición abierta no cuesta nada (cada estrategia tiene su % y no se roban capital, salvo margen activado), así que ordenar por tiempo no hace ganar más aquí; añadida igualmente la métrica **«por hora en mercado»** (lo ganado por 1 % ÷ horas con posición) por si prefiere estrategias rápidas por razones de fuera. En sus PM (mensual, 126, 4/2/1, suelo 1): lo ganado por 1 % → 5,81 M / −15,9 % (hoy 1A 4 · 3A 2 · 2A 1); EV por trade → 5,64 M; **por hora → 3A primera, 4,92 M / −14,4 %**; Sharpe → 5,93 M. Ayudas del freno reescritas paso a paso con el ejemplo 100.000 → 89.000 → mitad → 95.000 → completo. **Fixed ratio medido** (modelo lineal sobre su R por 1 %, reparto 4/2/1): base 700 $ y delta 2.000 → 1,14 M con DD −12,6 %, frente a ~1,5 M del compound fijo a esa misma caída; con delta 5.000-20.000 → 490 k, 270 k, 160 k con la misma DD −12,6 %; base 1.000/delta 10.000 → 516 k con DD −17,7 % (peor que el 7 % fijo en todo). Es un calendario que baja el % según crece la cuenta (acaba al 2 %): peor que el compound a igual caída y peor que la rotación. Backend reiniciado 17:18 (ocioso).
- **Paso 3, los tres gráficos en fila** (Jaume, 17:30, «no quiero un gráfico tan masivamente grande»; commit `ui(crudo): paso 3…`): rejilla 2fr/1fr/1fr: caminos según los locates (el grande, con la banda del rango elegido en la tabla), recorridos del bootstrap y las dos distribuciones apiladas. `SpaghettiChart`/`DistributionChart` (robustez) admiten `width`/`height` y en el crudo se miden con `useAncho` para dibujarse a tamaño real (viewBox = ancho de la columna) en vez de escalar el viewBox de 900 (que a un tercio de ancho dejaba letras de 3 px). Lo demás (Edge) sigue igual. **Pregunta del margen (17:35): «con margen y BP llego al 80 % de exposición, ¿Sage lo permite?»** Sí: la exposición es nocional/equity y el margen de Sage exige mucho menos que el nocional salvo por debajo de 2,50 $ (largos 25 %; cortos ≥ 5 $ el mayor de 30 % o 5 $/acción; 2,50–5 $ el 100 %; < 2,50 $ 2,50 $/acción): un 80 % en cortos de 5–15 $ usa un 40–80 % de la capacidad. Lo que comprueba el motor es la SUMA de exigencias contra el equity del día, y lo que corta es lo que no cabe; el Stat «Margen y BP» de Visión da el pico de uso. Caveats para él: son las reglas de la FAQ de Sage (sep-2026), intradía; un valor en lista especial (HTB, margen 100 %) sería más estricto en la realidad.
- **Última ronda del día** (`ac1138f`; Jaume, 20:00, auditando: «bootstrap y caminos del mismo tamaño y grosor; ¿seguro que margen y BP están bien? en real pocas veces me deja tener tantas posiciones; ¿dónde pongo el factor 1,25?; el paso 5 sigue con Kelly, creía que la habíamos descartado; el rendimiento de cada estrategia en curvas, estilo osciladores; y sé sincero sobre si hay algo que gane al compound»). (1) Paso 3: `LinesChart` admite `width` y se mide con `useAncho`; rejilla 2fr/2fr/1fr, caminos y bootstrap con la misma altura (300/278) y grosor. (2) Margen, repasado entero (`margen.py` contra la FAQ de Sage): la exigencia por posición, el tramo por precio y lado, la suma contra el equity del día y el corte en la vela de la decisión están bien; lo que no está en la simulación es lo que hace que en real «no le deje tener tantas posiciones»: la **lista especial (HTB)** en la que caen casi todos los gappers que vende en corto, donde el bróker exige el 100 % del valor aunque coticen a más de 5 $. Añadido el bróker **«SageTrader estricto (cortos 100 %)»** (`BROKERS["sagetrader_estricto"]`: todos los cortos ≥ 2,50 $ al 100 %, por debajo 2,50 $/acción; largos 25 %), test `test_broker_estricto_exige_el_100_por_ciento_a_todos_los_cortos`, y la ayuda del margen del paso 1 explica exposición (nocional/equity) vs margen (lo que exige el bróker). Medido en sus PM: estricto 74 saltados por margen frente a 66 del normal, y el MISMO pico de exposición (77 %): con posiciones del 1–4 % del equity ni al 100 % se agota el BP; si en real le frenan es por otra cosa (locates que no llegan, BP intradía distinto, límites por valor). (3) Bloque A: botón **«Usar ×k»** en cada fila (`usarNivel` en `CrudoTab`: multiplica `pctComun`/`pctPor` por el factor y recalcula el paso 1 solo; la fila ×1 dice «en uso»); la ayuda dice a las claras que la estrategia NO puede decirte cuánto poner: por debajo de Kelly, más es siempre más retorno y más caída, y el factor se elige por la DD p95 que se traga. Verificado en el navegador: ×1,25 → 1 %/1 % pasan a 1,25/1,25 y se recalcula solo. (4) Bloque B: `Nota` de aviso cuando el patrón es plano (la rotación no puede cambiar nada), y el **oscilador** «Rendimiento rodante por estrategia (lo que mira el ranking)»: `Rotacion.puntuaciones_diarias()` → `scores_daily {dates, por_estrategia}` en el informe, la puntuación de cada estrategia a cada día con la misma ventana y métrica del ranking (None hasta tener ventana completa), test `test_puntuaciones_diarias_coinciden_con_el_ranking`; en la UI un `LinesChart` debajo de las cuatro curvas. (5) **Paso 5 sin Kelly** (`PasoCuentaReal.tsx` reescrito): fichero o pegado → operaciones, PnL, curva real sobre los caminos del paso 3 (paridad simulado vs real), el freno del paso 4 aplicado a la curva real, y la tarjeta «Siguiente periodo — lo que toca poner en tu cuenta»: total y % por estrategia = los pesos del paso 4 (rotación `hoy` o los del paso 1) sin freno y con freno (× el multiplicador que el freno tenga hoy sobre SU curva); la petición viaja con `risk_mode notional`, `kelly_mult 1`, `cap_pct 0` y la UI ignora los campos de Kelly del endpoint (que sigue devolviéndolos). Suite **1.455 verde / 115 skipped**; `tsc` limpio; backend reiniciado 21:44 (ocioso 30 s). Bitácora: quitadas mis 4 líneas del navegador (17:53 y 21:47–21:51, las «TTP» a 100.000); las de 20:57–21:27 (4 estrategias a 120.000 y las PM con locates 1–50) son suyas.
- **«¿Detrending de cada estrategia para el gráfico, para ver cuál mejora respecto a la otra?»** (Jaume, 21:55). El oscilador ya es una ventana rodante (lo ganado por 1 % en las últimas X sesiones), no una curva acumulada: no lleva tendencia que quitar, es «la velocidad» de cada una. Quitarle a cada estrategia su propia media enseñaría «quién está mejor que SU normal», que es otra pregunta que la del ranking («quién gana más por cada 1 % ahora»). Medido ordenar por eso (modelo rápido, mensual, 126, 4/2/1): en crudo ×841 (1A primera el 92 % de los meses); **centrado (menos su media hasta la fecha) ×435; z-score ×398; cambio respecto a la ventana anterior ×372; cambio en 21 sesiones ×536; fijo ×651**: con 1A a 21 % por 1 % en 126 sesiones (rango 11–42) frente a 8,8 % de 2A (0,6–23) y 15 % de 3A, el «detrending» quita justo la información que hace ganar y da el primer puesto a 2A/3A el 69 % de los meses por estar «por encima de lo suyo». Respuesta: no para el ranking; como vista solo si él la quiere (no añadida).
- **Por qué a Jaume «el freno degrada una barbaridad» y «la rotación no mejora el compound» (sus corridas de las 21:10-21:27, reproducidas)**: su configuración de la noche es 5/2,5/2,5 % sobre 10.000 $, locates aleatorios **1–50 $** con puerta 4,2 %, **300 $/mes de gastos fijos**, freno 10/×0,5/5 y rotación con ventana de **15 sesiones** semanal (5/3/2). Fijo 1,33 M con DD −27,4 %. (1) Freno → 85 k: se frena el 2024-01-10 (día 7, −11 %) y **no suelta hasta el 2025-06-12** (71 % de los días a mitad de tamaño, 6 episodios) porque a mitad de tamaño la cuenta no puede con **300 $/mes sobre 10.000 $ (3 %/mes, 36 %/año)**: 2024 −21 % frenado vs +57 % fijo. Sin gastos fijos: fijo 2,14 M / −21,8 % y freno 1,11 M / −15,8 % (23 % de días frenado): el trueque normal. Las dos juntas → 16,6 k (92 % de días frenado). (2) Rotación 15 sesiones/semanal → 1,02 M: **63 cambios de ranking en 140 rebalanceos** = persigue ruido; con ventana 126/mensual y 5/3/2 → 1,28 M (empate: 2A pasa de 2,5 a 3 o 2), con **6/3/1 → 1,70 M (+28 %) con la MISMA caída**, 7 cambios en 33 rebalanceos. La ventaja de la rotación está en ventanas de 6 meses con un patrón que concentre; con 15-30 sesiones no la hay. Para decírselo: los gastos fijos se ponen en la cuenta que va a tener, no en 10.000 $ de prueba, y el freno con gastos fijos altos se queda enganchado.
- **«¿Y si usásemos Markov para predecir a qué estrategia darle más peso?»** (Jaume, 22:30). Medido walk-forward (solo con datos hasta la fecha), modelo rápido mensual, 4/2/1 sobre su corrida de la mañana (fijo ×651; rotación por ranking con ventana 126 ×841). **A. Cadena de Markov sobre «la mejor del mes»** (matriz de transición estimada con los meses pasados) → ×724. La matriz lo explica: sea quien sea la mejor este mes, el mes que viene la mejor es 1A el 62–75 % de las veces (1A → 1A 62 %, 3A → 1A 75 %): el estado no importa, la cadena solo redescubre las frecuencias de largo plazo (1A mejor 22 de 33 meses, 2A 3, 3A 8; 2A la peor 20 de 33) con más ruido. **B. Markov bueno/malo por estrategia** (mes por encima/debajo de su mediana; E[ret] = P·μ) → ×904, +7 % sobre la rotación con la MISMA asignación media (3,94/1,25/1,82 vs 3,88/1,25/1,88): ruido de 33 meses; la persistencia mes a mes es 56–62 % (moneda: 50 %). **C. HMM de 2 regímenes por estrategia** (hmmlearn, reajustado cada mes, 3 semillas): patrón por E[ret] ×795; proporcional ×797 (DD −14,6); ranking 126 + apagar la de régimen malo ×841 (nunca apaga); actualizado cada DÍA ×808 / ×775 (87 cambios). Los regímenes que encuentra duran ~10 días (μ 0,26 vs 0,11 % por 1 % en 1A) y elige la misma primera que la ventana de 126 el 81 % de los meses. Conclusión: ninguna forma de Markov sobre los propios retornos gana a la ventana larga; para que un modelo de regímenes aportara haría falta información EXTERNA con regímenes de meses (nº de gappers, volumen small-cap, VIX…), no las curvas de las estrategias. La intuición de Jaume («dar más a las que más van») es correcta en lo grande y falsa en lo pequeño: la ventaja viene de CONCENTRAR en la de más edge (frontera: todo a 1A ×23 M), no de cambiar; y en un mes «la que más va» es suerte (1A gana a 2A por 2,1 % ± 2,9 %: t 0,7), en seis meses es edge (13 % ± 7 %: t 1,8 → 92 % de los meses).
