...
- **Regla de bolsillo FUERA** (Jaume: «lo que esté en el EV es lo que manda; el margen lo pongo yo mentalmente» y «no quiero el tramo más flojo»): primero se le quitó el × 2/3 y luego se quitó entera del encabezado de Charts. Quedan el tope según el EV GENERAL de la estrategia (arriba, «por cada 1 $ de precio») y el tope por tramo con el EV de cada rango (columna TOPE EV). Ni la puerta ni el `/evf` llevaron nunca margen (`entra = ev > fade`). Y «Volumen del pico» en la familia de volumen del builder: la casilla en millones con la M, guardado en acciones. Dudas suyas aclaradas: una acción de 1 $ con 0,3-15 paga ≥ 20 $ 3 días de cada 100 (la cola es a propósito: DCX 0,73 $ → 8,63 y WHLR 2,5 $ → 24,4 en el extracto real); la edad del pico se cuenta desde la vela del techo hasta la actual y crece (no es la ventana).
